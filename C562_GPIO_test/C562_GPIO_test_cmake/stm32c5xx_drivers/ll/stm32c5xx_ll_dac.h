...
#define LL_DAC_TRIGGER_LPTIM1_CH1   ( DAC_CR_TSEL1_3 |                  DAC_CR_TSEL1_1 | DAC_CR_TSEL1_0 )
...
/*!< The selected DAC channel output is on mode sample-and-hold. Mode sample-and-hold requires an external capacitor,
refer to description of function @ref LL_DAC_ConfigOutput() or @ref LL_DAC_SetOutputMode(). */
...
/*!< The selected DAC channel output is connected to external pin.
     Note: Depending on other parameters (mode normal or sample and hold, output buffer state), output can also be
     connected to on-chip peripherals, refer to ref manual or comments of function @ref LL_DAC_SetOutputConnection(). */
...
/*!< The selected DAC channel output is connected to on-chip peripherals (via internal paths).
     Note: Depending on other parameters (mode normal or sample and hold, output buffer state), output can also be
     connected to external pin, refer to ref manual or comments of function @ref LL_DAC_SetOutputConnection(). */
...
/**
  ...
  */
#define LL_DAC_WRITE_REG(instance, reg, value) WRITE_REG((instance)->reg, (value))
...
/**
  ...
  */
#define LL_DAC_READ_REG(instance, reg) READ_REG((instance)->reg)
...
{
  MODIFY_REG(dacx->MCR, DAC_MCR_HFSEL, high_freq_mode);
}
...
/**
  ...
  *         @arg @ref LL_DAC_TRIGGER_LPTIM1_CH1
...
__STATIC_INLINE void LL_DAC_SetSampleAndHoldSampleTime(DAC_TypeDef *dacx, uint32_t dac_channel, uint32_t sample_time)
{
  ...
  MODIFY_REG(*preg, DAC_SHSR1_TSAMPLE1, sample_time);
}
...
{
  return ((((READ_REG(dacx->CR)) & ((DAC_CR_TEN1 | DAC_CR_TSEL1) << (dac_channel & DAC_CR_CHX_BITOFFSET_MASK)))
           == (DAC_CR_TEN1 << (dac_channel & DAC_CR_CHX_BITOFFSET_MASK))) ? 1UL : 0UL);
}
...
/**
  ...
  * @rmtoll
  *  SR       CAL_FLAG1  CAL_FLAG2    LL_DAC_IsActiveFlag_CAL
  * @param  dacx DAC instance
  * @param  dac_channel This parameter can be one of the following values:
  *         @arg @ref LL_DAC_CHANNEL_1
  * @retval State of bit (1 or 0).
...
{
  return ((READ_BIT(dacx->SR, LL_DAC_FLAG_CAL1 << (dac_channel & DAC_CR_CHX_BITOFFSET_MASK))
...
/**
  ...
  * @rmtoll
  *  SR       "all bits in Flag"        LL_DAC_IsActiveFlag_DMAUDR
  * @param  dacx DAC instance
...
{
  return ((READ_BIT(dacx->SR, flag) == (flag)) ? 1UL : 0UL);
}
...
/**
  ...
  * @rmtoll
  *  SR       DMAUDR1  DMAUDR2       LL_DAC_ClearFlag_DMAUDR
  * @param  dacx DAC instance
...
{
  SET_BIT(dacx->SR, LL_DAC_FLAG_DMAUDR1);
}
...
{
  return ((READ_BIT(dacx->CR, mask) == (mask)) ? 1UL : 0UL);
}
