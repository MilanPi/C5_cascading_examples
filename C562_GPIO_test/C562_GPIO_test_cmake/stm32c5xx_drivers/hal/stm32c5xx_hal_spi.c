...
/** @addtogroup SPI
  * @{
  ...
  - Initialize the SPI according to the associated handle with HAL_SPI_Init()
    Note: The SPI clock can be enabled in the HAL_SPI_Init() function if USE_HAL_SPI_CLK_ENABLE_MODEL
  ...
  Note: HAL_SPI_RegisterTxHalfCpltCallback(), HAL_SPI_RegisterRxHalfCpltCallback() and
        HAL_SPI_RegisterTxRxHalfCpltCallback() applies only in DMA mode.
...
------------------------------ | -----------------| ------------------| -------------------------------------------
USE_HAL_SPI_MODULE             | hal_conf.h       |         1U        | Enable the HAL SPI module.
USE_HAL_SPI_DMA                | hal_conf.h       |         1U        | Enable the use of DMA in the HAL driver
USE_HAL_SPI_GET_LAST_ERRORS    | hal_conf.h       |         0U        | Enable retrieving last processes error codes
USE_HAL_SPI_REGISTER_CALLBACKS | hal_conf.h       |         0U        | Enable the register callbacks assert
USE_HAL_SPI_USER_DATA          | hal_conf.h       |         0U        | Add an user data inside HAL SPI handle
USE_HAL_SPI_CRC                | hal_conf.h       |         1U        | Enable the use of CRC feature inside driver
USE_HAL_SPI_CLK_ENABLE_MODEL   | hal_conf.h       | HAL_CLK_ENABLE_NO | Enable peripheral clock inside the HAL_SPI_Init
...
/**
  * @brief Check if the readypin polarity is type of hal_spi_ready_pin_polarity_t.
  */
...
#if defined(USE_HAL_SPI_CLK_ENABLE_MODEL) && (USE_HAL_SPI_CLK_ENABLE_MODEL == HAL_CLK_ENABLE_PERIPH_ONLY)
...
hal_status_t HAL_SPI_SetConfigCRC(hal_spi_handle_t *hspi, const hal_spi_crc_config_t *p_config)
{
   ...
   */
  if (length_crc == HAL_SPI_CRC_LENGTH_32_BIT)
  {
...
/**
  ...
  * @param hspi Pointer to a \ref hal_spi_handle_t structure which contains
  *             the SPI instance.
  * @retval hal_spi_master_keep_io_state_status_t SPI mster keep IO state feature status.
  */
...
hal_status_t HAL_SPI_SetMode(hal_spi_handle_t *hspi, const hal_spi_mode_t mode)
{
  ...
  if ((IS_BIT_SET(cfg2_reg_value, SPI_CFG2_SSM))
      && (((mode == HAL_SPI_MODE_MASTER) && (IS_BIT_CLR(cfg2_reg_value, SPI_CFG2_SSIOP)))
          || ((mode == HAL_SPI_MODE_SLAVE)  && (IS_BIT_SET(cfg2_reg_value, SPI_CFG2_SSIOP)))))
  {
...
/**
  ...
  * @param count_packet Amount of data to be sent in bytes.
...
hal_status_t HAL_SPI_Abort(hal_spi_handle_t *hspi)
{
  ...
  if (LL_SPI_IsActiveMasterTransfer((SPI_TypeDef *)((uint32_t)hspi->instance)) == 1U)
  {
    ...
    } while (LL_SPI_IsEnabledIT_EOT((SPI_TypeDef *)((uint32_t)hspi->instance)) == 1U);
    ...
    } while (LL_SPI_IsActiveMasterTransfer((SPI_TypeDef *)((uint32_t)hspi->instance)) == 1U);
  ...
  if (LL_SPI_IsEnabledDMAReq_TX((SPI_TypeDef *)((uint32_t)hspi->instance)) == 1U)
  {
  ...
  if (LL_SPI_IsEnabledDMAReq_RX((SPI_TypeDef *)((uint32_t)hspi->instance)) == 1U)
  {
...
hal_status_t HAL_SPI_Abort_IT(hal_spi_handle_t *hspi)
{
  ...
  if (((hspi->global_state != HAL_SPI_STATE_IDLE) && (hspi->hdma_tx->p_xfer_abort_cb == NULL)
       && (hspi->hdma_rx->p_xfer_abort_cb == NULL)))
  {
    SPI_AbortTransfer(hspi);
...
void HAL_SPI_IRQHandler(hal_spi_handle_t *hspi)
{
  ...
  /* SPI in SUSPEND mode  ----------------------------------------------------*/
  if (IS_BIT_SET(it_flag, SPI_SR_SUSP) && IS_BIT_SET(it_source, SPI_SR_EOT))
  {
  ...
  if ((IS_BIT_CLR(trigger, SPI_SR_OVR) && IS_BIT_CLR(trigger, SPI_SR_UDR))
      && (IS_BIT_SET(trigger, SPI_SR_DXP)))
  {
    hspi->p_tx_isr(hspi);
    hspi->p_rx_isr(hspi);
    handled = 1UL;
  ...
  if ((IS_BIT_CLR(trigger, SPI_SR_OVR) && IS_BIT_SET(trigger, SPI_SR_RXP))
      && (IS_BIT_CLR(trigger, SPI_SR_DXP)))
  {
    hspi->p_rx_isr(hspi);
    handled = 1UL;
  }
  ...
  if ((IS_BIT_CLR(trigger, SPI_SR_UDR) && IS_BIT_SET(trigger, SPI_SR_TXP))
      && (IS_BIT_CLR(trigger, SPI_SR_DXP)))
  {
    hspi->p_tx_isr(hspi);
    handled = 1UL;
  ...
  if (IS_BIT_SET(trigger, SPI_SR_EOT))
  {
    ...
    if (IS_BIT_CLR(((SPI_TypeDef *)((uint32_t)hspi->instance))->CFG1, SPI_CFG1_TXDMAEN | SPI_CFG1_RXDMAEN))
    {
  ...
  if ((trigger & (SPI_SR_MODF | SPI_SR_OVR | SPI_SR_TIFRE | SPI_SR_UDR)) != 0UL)
  {
    ...
    {
#if defined (USE_HAL_SPI_GET_LAST_ERRORS) && (USE_HAL_SPI_GET_LAST_ERRORS == 1)
      SET_BIT(hspi->last_error_codes, HAL_SPI_ERROR_OVR);
#endif /* USE_HAL_SPI_GET_LAST_ERRORS */
    ...
    {
#if defined (USE_HAL_SPI_GET_LAST_ERRORS) && (USE_HAL_SPI_GET_LAST_ERRORS == 1)
      SET_BIT(hspi->last_error_codes, HAL_SPI_ERROR_MODF);
#endif /* USE_HAL_SPI_GET_LAST_ERRORS */
    ...
    {
#if defined (USE_HAL_SPI_GET_LAST_ERRORS) && (USE_HAL_SPI_GET_LAST_ERRORS == 1)
      SET_BIT(hspi->last_error_codes, HAL_SPI_ERROR_FRE);
#endif /* USE_HAL_SPI_GET_LAST_ERRORS */
    ...
    {
#if defined (USE_HAL_SPI_GET_LAST_ERRORS) && (USE_HAL_SPI_GET_LAST_ERRORS == 1)
      SET_BIT(hspi->last_error_codes, HAL_SPI_ERROR_UDR);
#endif /* USE_HAL_SPI_GET_LAST_ERRORS */
    ...
    if ((IS_BIT_SET(cfg1_reg_value, SPI_CFG1_RXDMAEN)) || (IS_BIT_SET(cfg1_reg_value, SPI_CFG1_TXDMAEN)))
    {
      if (IS_BIT_SET(cfg1_reg_value, SPI_CFG1_RXDMAEN))
      {
        ...
        if (hspi->hdma_rx != NULL)
        {
          ...
          {
#if defined (USE_HAL_SPI_GET_LAST_ERRORS) && (USE_HAL_SPI_GET_LAST_ERRORS == 1)
            SET_BIT(hspi->last_error_codes, HAL_SPI_ERROR_ABORT);
#endif /* USE_HAL_SPI_GET_LAST_ERRORS */
          }
        }
      }
      if (IS_BIT_SET(cfg1_reg_value, SPI_CFG1_TXDMAEN))
      {
        /* Disable the SPI DMA requests */
        LL_SPI_DisableDMAReq_TX((SPI_TypeDef *)((uint32_t)hspi->instance));
...
/** @addtogroup SPI_Exported_Functions_Group8 Peripheral current frequency, state and errors functions
  ...
  *  - HAL_SPI_GetLastErrorsCodes() API to retrieve the error codes in case of HAL_ERROR return
...
/**
  ...
  * @param hdma DMA handle.
  * @warning When this callback is executed, User Abort complete call back is called only if no
...
static hal_status_t SPI_CloseTransfer(hal_spi_handle_t *hspi)
{
  ...
  if (hspi->global_state != HAL_SPI_STATE_TX_ACTIVE)
  {
    ...
    if (LL_SPI_IsEnabledCRC((SPI_TypeDef *)((uint32_t)hspi->instance)) != 0UL)
    {
      ...
      {
#if defined(USE_HAL_SPI_GET_LAST_ERRORS) && (USE_HAL_SPI_GET_LAST_ERRORS == 1)
        SET_BIT(hspi->last_error_codes, HAL_SPI_ERROR_CRC);
#endif /* USE_HAL_SPI_GET_LAST_ERRORS */
        LL_SPI_ClearFlag_CRCERR((SPI_TypeDef *)((uint32_t)hspi->instance));
