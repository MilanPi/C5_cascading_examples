...
/** @addtogroup USART
  * @{
  ...
  - Declare a hal_usart_handle_t handle structure, for example:
      hal_usart_handle_t husart;

  - Initialize the USART low level resources :
    - Enable the USART interface clock if you have set USE_HAL_USART_CLK_ENABLE_MODEL to HAL_CLK_ENABLE_PERIPH_ONLY,
      in this case HAL_USART_Init() will enable the clock.
    - USART pins configuration:
      - Enable the clock for the USART GPIOs
      - Configure these USART pins as alternate function.
    - NVIC configuration if one needs to use interrupt process (HAL_USART_Transmit_IT()
              and HAL_USART_Receive_IT(), HAL_USART_TransmitReceive_IT() and their _Opt equivalent APIs):
      - Configure the USART interrupt priority.
      - Enable the NVIC USART IRQ Channel.
    - DMA Configuration if one needs to use DMA process (HAL_USART_Transmit_DMA()
              and HAL_USART_Receive_DMA(), HAL_USART_TransmitReceive_DMA() and their _Opt equivalent APIs):
      ...
      @warning In DMA configuration, one also needs to enable the USART IRQ to complete the DMA transfer.

  - Initialize the USART with HAL_USART_Init() and by selecting an instance, for example:
        HAL_USART_Init(husart, HAL_USART1);
  - Declare a hal_usart_config_t structure, for example:
  ...
  - Apply the configuration with <b>HAL_USART_SetConfig(&my_config)</b>

  - if needed, to configure and enable/disable the USART to wake up the MCU from stop mode, resort to UART API's
       <b>HAL_UART_SetStopModeWakeUpAddress()</b>, <b>HAL_UART_EnableStopMode()</b> and
       <b>HAL_UART_DisableStopMode()</b> in casting the USART handle to UART type <b>hal_usart_handle_t</b>.

## Callbacks definition in Interrupt or DMA mode:

  When the compilation define USE_HAL_USART_REGISTER_CALLBACKS is set to 1U, the user can configure dynamically the
...
-------------------------------- | -----------------| ------------------| -------------------------------------------
USE_HAL_USART_MODULE             | hal_conf.h       |         1         | Enable the HAL USART module.
USE_HAL_USART_REGISTER_CALLBACKS | hal_conf.h       |         0         | Enable the register callbacks
USE_HAL_USART_CLK_ENABLE_MODEL   | hal_conf.h       | HAL_CLK_ENABLE_NO | Enable peripheral clock in the HAL_USART_Init
USE_HAL_USART_USER_DATA          | hal_conf.h       |         0         | Add an user data inside HAL USART handle
USE_HAL_USART_GET_LAST_ERRORS    | hal_conf.h       |         0         | Add error member inside HAL USART handle
USE_HAL_USART_DMA                | hal_conf.h       |         1         | Enable DMA code inside HAL USART
USE_HAL_USART_FIFO               | hal_conf.h       |         0         | Enable FIFO code inside HAL USART
...
/*! USART TX or RX enable acknowledge time-out value      */
...
/**
  ...
  */
#define IS_USART_CLOCK_POLARITY(clock_polarity) (((clock_polarity) == HAL_USART_CLOCK_LOW)  \
                                                 || ((clock_polarity) == HAL_USART_CLOCK_HIGH))
/**
...
/** @defgroup USART_Private_Functions USART Private Functions
  * @{
  */
#if defined(USE_HAL_USART_CLK_ENABLE_MODEL) && (USE_HAL_USART_CLK_ENABLE_MODEL == HAL_CLK_ENABLE_PERIPH_ONLY)
...
/** @addtogroup USART_Exported_Functions_Group1
  * @{
This subsection provides a set of functions allowing to initialize and deinitialize the USART in
synchronous mode.
  - Call the function HAL_USART_Init() to initialize the selected USART handle and associate an instance.
  - Call the function HAL_USART_DeInit() to de-initialize the given HAL USART instance by stopping any ongoing process
  and resetting the state machine .
  */

/**
  * @brief Initialize the USART handler for the associated instance.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which will contains the USART instance.
  ...
  * @retval HAL_ERROR USART semaphore creation is failed (USE_HAL_MUTEX is set to 1)
  */
...
/** @addtogroup USART_Exported_Functions_Group2
  * @{
This subsection provides a set of functions allowing to configure the USART in synchronous mode.
  ...
  - If needed, after calling HAL_USART_SetConfig(), one can modify its configuration by using unitary
  ...
  @warning
    ...
    call HAL_USART_SetConfig() to modify it.
...
void HAL_USART_GetConfig(const hal_usart_handle_t *husart, hal_usart_config_t *p_config)
{
  ...
  p_config->clock_polarity = (hal_usart_clock_polarity_t)(uint32_t)(reg_temp & (LL_USART_POLARITY_HIGH));
  p_config->clock_phase = (hal_usart_clock_phase_t)(uint32_t)(reg_temp & (LL_USART_PHASE_2_EDGE));
...
/**
  * @brief Set the Baud Rate configuration set as parameter into the handler instance registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
...
/** @addtogroup USART_Exported_Functions_Group3
  * @{
  ...
  Prior to use the fifo mode feature, one has to configure one's instance in synchronous
  mode with HAL_USART_SetConfig(). All these functions are usable only if USE_HAL_USART_FIFO is set to 1.
  ...
  The feature has been designed to be used following the procedure:
    ...
    - Start process, e.g: HAL_USART_Receive()
  */
...
hal_status_t HAL_USART_EnableFifoMode(hal_usart_handle_t *husart)
{
  ...
  ASSERT_DBG_PARAM(IS_UART_FIFO_INSTANCE(p_usartx));
...
/** @addtogroup USART_Exported_Functions_Group5
  * @{
  ...
  Prior to configure advanced features, one has to configure one's instance in synchronous
  mode with HAL_USART_SetConfig().

  One can configure some features by calling the associated functions:
    - HAL_USART_SetSlaveSelect(): Set the slave select either software or hardware using USART NSS pin
    - HAL_USART_GetSlaveSelect(): Get the slave select configuration either software or hardware using USART NSS pin
  */
/**
...
/** @addtogroup USART_Exported_Functions_Group6 DMA Configuration functions
  * @{
  This subsection provides a set of functions allowing to link the HAL USART handle to a Tx and Rx DMA handler
...
/** @addtogroup USART_Exported_Functions_Group7 Callbacks Register functions
  * @{
  ...
  Prior to configure the Callbacks, one has to configure one's instance in synchronous
  mode with HAL_USART_SetConfig().
...
/** @addtogroup USART_Exported_Functions_Group8 IO operation functions
  *  @{
  ...
  For Abort services based on interrupts (HAL_USART_Abort_IT()), an Abort Complete Callbacks is provided:
    - HAL_USART_AbortCpltCallback()

  In Non-Blocking mode transfers, possible errors are split into 2 categories.
  - Error is considered as Recoverable and non blocking : Transfer could go till end, but error severity is
    to be evaluated by user :
  ...
  - Error is considered as Blocking : Transfer could not be completed properly and is aborted.
    ...
    In all cases, HAL_USART_ErrorCallback() user callback is executed and error code is set to allow user to identify
    error type if USE_HAL_USART_GET_LAST_ERRORS=1.
  */
    ...
    if (LL_USART_IsEnabledDMAReq_TX(p_usartx) != 0U)
    {
      ...
      if (husart->hdma_tx != NULL)
      {
        if (husart->hdma_tx->global_state == HAL_DMA_STATE_ACTIVE)
        {
          ...
          {
            husart->hdma_tx->p_xfer_abort_cb = NULL;
          }
          else
          {
            abort_cplt = 0U;
    ...
    if (LL_USART_IsEnabledDMAReq_RX(p_usartx) != 0U)
    {
      ...
      if (husart->hdma_rx != NULL)
      {
        if (husart->hdma_rx->global_state == HAL_DMA_STATE_ACTIVE)
        {
          ...
          {
            husart->hdma_rx->p_xfer_abort_cb = NULL;
          }
          else
          {
            abort_cplt = 0U;
...
/** @addtogroup USART_Exported_Functions_Group11
  * @{
  This subsection provides functions allowing to set user specific data to a USARTx instance:
    - HAL_USART_SetUserData(): Set user data in handler.
    - HAL_USART_GetUserData(): Get user data from handler.
  */
...
hal_status_t USART_Start_Receive_DMA(hal_usart_handle_t *husart, uint8_t *p_data, uint32_t size, uint32_t interrupts)
{
  ...
  if (husart->usart_mode == HAL_USART_MODE_MASTER)
  {
    ...
    if (husart->hdma_tx != NULL)
    {
      ...
      husart->hdma_tx->p_xfer_halfcplt_cb = NULL;
