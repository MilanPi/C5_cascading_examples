...
/** @addtogroup UART
  * @{
  ...
  @note In the following documentation one considers USARTx as a reflection of every UART instances,
        USART instances, and LPUART instances as well.

  ## The UART HAL driver can be used as follows:

  - Declare a hal_uart_handle_t handle structure, for example:
      hal_uart_handle_t huart;

  - Initialize the UART low-level resources:
    - Enable the USARTx interface clock unless you have set USE_HAL_UART_CLK_ENABLE_MODEL > HAL_CLK_ENABLE_NO, in this
      case HAL_UART_Init() enables the clock.
    - UART pins configuration:
      - Enable the clock for the UART GPIOs
      - Configure these UART pins as alternate function.
    - NVIC configuration if one needs to use interrupt process (HAL_UART_Transmit_IT()
              and HAL_UART_Receive_IT(), ... APIs):
      - Configure the USARTx interrupt priority.
      - Enable the NVIC USARTx IRQ Channel.
    - DMA configuration if one needs to use DMA process (HAL_UART_Transmit_DMA()
      ...
      @note In DMA Tx configuration, one also needs to enable the USARTx IRQ to complete the DMA transfer.

  - Initialize the UART with HAL_UART_Init() and by selecting an instance, for example:
        HAL_UART_Init(huart, HAL_UART1);
  - Declare a hal_uart_config_t structure, for example:
  ...
  - In the configuration structure,
    ...
    flow control, direction (Receiver/Transmitter), over-sampling and one-bit sampling.
  ...
  with a set of different configuration functions.

## Callbacks definition in interrupt or DMA mode:

  When the compilation define USE_HAL_UART_REGISTER_CALLBACKS is set to 1U, the user can dynamically configure the
...
------------------------------- | -----------------| ------------------| -------------------------------------------
USE_HAL_UART_MODULE             | hal_conf.h       |         1         | Enable the HAL UART module.
USE_HAL_UART_REGISTER_CALLBACKS | hal_conf.h       |         0         | Enable the register callbacks
USE_HAL_UART_CLK_ENABLE_MODEL   | hal_conf.h       | HAL_CLK_ENABLE_NO | Enable peripheral clock in the HAL_UART_Init
USE_HAL_UART_USER_DATA          | hal_conf.h       |         0         | Add a user data inside HAL UART handle
USE_HAL_UART_GET_LAST_ERRORS    | hal_conf.h       |         0         | Add error member inside HAL UART handle
USE_HAL_UART_DMA                | hal_conf.h       |         1         | Enable DMA code inside HAL UART
...
/**
  ...
  */
#define IS_UART_OVERSAMPLING(sampling) (((sampling) == LL_USART_OVERSAMPLING_16)    \
                                        || ((sampling) == LL_USART_OVERSAMPLING_8))
...
/**
  ...
  * @retval SET (sampling is valid) or RESET (sampling is invalid)
  */
#define IS_LPUART_OVERSAMPLING(sampling) ((sampling) == LL_USART_OVERSAMPLING_16)
...
/**
  ...
  * @param wakeup UART wake-up method .
...
/**
  * @brief Check if UART instance is enabled. If yes, disable it.
...
/**
  * @brief Check if UART instance needs to be re-enabled.
  * @param handle specifies the UART Handle
  */
...
/**
  * @brief Retrieve UART instance from handle
...
/** @defgroup UART_Private_Functions UART Private Functions
  * @{
  */
#if defined(USE_HAL_UART_CLK_ENABLE_MODEL) && (USE_HAL_UART_CLK_ENABLE_MODEL == HAL_CLK_ENABLE_PERIPH_ONLY)
...
/** @brief  Report the UART mask to apply to retrieve the received data
  ...
  *         DMA transfers.
  * @retval HAL_OK UART RDR Mask has correctly been calculate.
  * @retval HAL_ERROR UART RDR Mask can't be calculate because of a mismatch
...
/** @addtogroup UART_Exported_Functions_Group1
  * @{
This subsection provides a set of functions allowing to initialize and deinitialize the USARTx in
asynchronous mode.
  - Call the function HAL_UART_Init() to initialize the selected USARTx handle and associate an instance.
  - Call the function HAL_UART_DeInit() to de-initialize the given HAL UART instance by stopping any ongoing process
...
This subsection provides a set of functions allowing to configure the USARTx in asynchronous mode.
  ...
  - If needed, after calling HAL_UART_SetConfig(), one can modify its configuration by using unitary
  ...
  @note
    - __Prescaler__: cannot be modified with a unitary configuration function as it impacts other parameters, one can
    call HAL_UART_SetConfig() to modify it.
    - __Over Sampling__: cannot be modified with a unitary configuration function as it impacts other parameters, one
    can call HAL_UART_SetConfig() to modify it.
  ...
  @note
    ...
    getter as well.
    - __Over Sampling__: As there is no unitary configuration function for this parameter, there is no unitary getter
    as well.
...
hal_uart_one_bit_sample_t HAL_UART_GetOneBitSample(const hal_uart_handle_t *huart)
{
  ...
  if (LL_USART_IsEnabledOneBitSample(p_uartx) == 1U)
  {
...
/** @addtogroup UART_Exported_Functions_Group3
  * @{
  This subsection provides a set of functions allowing to configure the USARTx instance in IRDA mode
  A set of functions is provided to use the IRDA feature:
  ...
  It is important to note that the HAL_UART_IRDA_SetConfig API can be called without calling
  HAL_UART_SetConfig beforehand. The HAL_UART_xxxx API are still available in IRDA and have to be used to communicate.
...
/** @addtogroup UART_Exported_Functions_Group4
  * @{
  This subsection provides a set of functions allowing to configure the USARTx instance in particular
  asynchronous modes.
  Prior to enable one of the following particular asynchronous modes, one has to configure one's instance in
  ...
  For the Multi Processor mode, once one enabled the mode, it is not active yet, one has to call
  HAL_UART_EnterMultiProcessorMuteMode() to enter in Mute. One can check if the UART is in Mute with
...
/**
  * @brief Enable the Multi Processor mode.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @note  This does not make the instance entering in Mute Mode. For this, use HAL_UART_EnterMultiProcessorMuteMode.
  * @retval HAL_OK UART instance has been correctly configured.
  */
...
/**
  * @brief Disable the Multi Processor Mode into the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @retval HAL_OK UART instance has been correctly configured.
  */
...
/**
  * @brief Return the Multi Processor Mode status according to the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @retval hal_uart_multi_processor_mode_status_t Current Multi Processor Mode status.
  */
...
/**
  * @brief For Multi Processor mode, set the Mute configuration passed in parameters into the handler instance
  *        registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
...
/**
  * @brief For Multi Processor mode, get the Mute configuration according to the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @param p_wakeup_config Pointer to a hal_uart_multi_processor_mode_wakeup_config_t.
  */
...
/**
  * @brief For Multi Processor Mode, request instance to enter in Mute.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
...
/**
  * @brief For Multi Processor Mode, return if the instance is in Mute.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @retval hal_uart_multi_processor_mode_mute_status_t Status of the Mute.
  */
...
/** @addtogroup UART_Exported_Functions_Group5
  * @{
  This subsection provides a set of functions allowing to configure advanced features for the USARTx instance.
  Please consider all advanced features might not be supported on all instances.
  Prior to configure advanced features, one has to configure one's instance in asynchronous
...
/** @addtogroup UART_Exported_Functions_Group6
  * @{
  This subsection provides a set of functions allowing to use auto baud rate feature for the USARTx instance.
  Prior to use auto baud rate feature, one has to configure one's instance in asynchronous
  mode with HAL_UART_SetConfig().
                                                ...
                                                hal_uart_auto_baud_rate_mode_t auto_baud_rate_mode)
{
  ...
  /* Auto Baud Rate can be configured only if not enabled */
...
/** @addtogroup UART_Exported_Functions_Group7
  * @{
  This subsection provides a set of functions allowing to use the stop mode feature for the USARTx instance.
  Prior to use the stop mode feature, one has to configure one's instance in asynchronous
  mode with HAL_UART_SetConfig().
  A set of functions is provided to use the stop mode feature:
    ...
    - HAL_UART_SetStopModeWkUpSource(): Set the source of the Wake Up flag
    - HAL_UART_GetStopModeWkUpSource(): Get the source of the Wake Up flag
    - HAL_UART_SetStopModeWkUpAddrLength(): Set the length of the address if the Wake Up source is a address
...
/**
  * @brief Set the Stop Mode Wake Up Source passed in parameters into the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @param source Stop Mode Wake Up source.
  * @note This API set the source of the WUF flag, it does not mean other Stop Mode sources won't wake up the USARTx
  *       instance.
  * @retval HAL_OK UART instance has been correctly configured.
  */
...
/**
  * @brief Get the Stop Mode Wake Up Source configuration according to the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @retval hal_uart_wakeup_source_t source of the Stop Mode Wake Up.
  */
...
/**
  * @brief Set the Stop Mode Wake Up Address Length passed in parameters into the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @param address_length Address Length to set.
  * @note This Api has to be used with HAL_UART_SetStopModeWkUpAddr()
  * @retval HAL_OK UART instance has been correctly configured.
  */
...
/**
  * @brief Get the Stop Mode Wake Up Address Length according to the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
...
/**
  * @brief Set the Stop Mode Wake Up Address passed in parameters into the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @param address Address to set.
  * @note This Api has to be used with HAL_UART_SetStopModeWkUpAddrLength()
  * @retval HAL_OK UART instance has been correctly configured.
  */
...
/**
  * @brief Get the Stop Mode Wake Up Address according to the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
...
/** @addtogroup UART_Exported_Functions_Group8
  * @{
  This subsection provides a set of functions allowing to use the fifo mode feature for the USARTx instance.
  Prior to use the fifo mode feature, one has to configure one's instance in asynchronous
  mode with HAL_UART_SetConfig().
...
hal_status_t HAL_UART_EnableFifoMode(hal_uart_handle_t *huart)
{
  ...
  ASSERT_DBG_PARAM(huart != NULL);
  p_uartx = UART_GET_INSTANCE(huart);
  ASSERT_DBG_PARAM(IS_UART_FIFO_INSTANCE(p_uartx));
  ASSERT_DBG_STATE(huart->global_state, HAL_UART_STATE_CONFIGURED);
  ASSERT_DBG_STATE(huart->rx_state, HAL_UART_RX_STATE_IDLE);
  ASSERT_DBG_STATE(huart->tx_state, HAL_UART_TX_STATE_IDLE);
...
/**
  ...
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @param tx_fifo_threshold Tx FIFO Threshold to applied.
  * @note  This feature is not available in LIN mode.
  * @retval HAL_OK UART instance has been correctly configured.
  */
...
/**
  ...
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @param rx_fifo_threshold Rx FIFO Threshold to applied.
  * @note  This feature is not available in LIN mode.
  * @retval HAL_OK UART instance has been correctly configured.
  */
...
/** @addtogroup UART_Exported_Functions_Group10
  * @{
  This subsection provides a set of functions allowing to link the HAL UART handle to a Tx and Rx DMA handler
...
/** @addtogroup UART_Exported_Functions_Group11
  * @{
  ...
  Prior to configure the Callbacks, one has to configure one's instance in asynchronous
  mode with HAL_UART_SetConfig().
  A set of functions is provided to configure the callbacks:
    ...
    - HAL_UART_RegisterWakeupCallback(): Set the wake up callback
 ...
 There are two modes of transfer:
  ...
  In Non-Blocking mode transfers, possible errors are split into 2 categories:
    - Error is considered as Recoverable and non blocking : Transfer could go till end, but error severity is
      ...
      If user wants to abort it, Abort services must be called by user.
...
/** @addtogroup UART_Exported_Functions_Group13
  * @{
  This subsection provides a set of functions allowing to use advanced IO operation on the USARTx
  instance in asynchronous modes.
  Prior to use advanced IO operations, one has to configure one's instance in asynchronous
  mode with HAL_UART_SetConfig().
...
hal_status_t HAL_UART_SendRequest(hal_uart_handle_t *huart, hal_uart_request_t request)
{
  ...
  LL_USART_SetRequest((p_uartx), (uint16_t)request);
...
/** @addtogroup UART_Exported_Functions_Group14
  * @{
  This subsection provides functions allowing to read the current frequency, state and last occurred
  errors of USARTx in asynchronous mode :
...
/** @addtogroup UART_Exported_Functions_Group18
  * @{
  ...
  functions. One can refer to the "How to use the UART HAL module driver" section to find the association between
  callbacks, registration function and default callback values.
  */
/**
...
/**
  ...
  * @param rx_event event which triggered the callback
  */
...
/**
  * @brief If not enabled, enables the UART instance and check acknowledge bits.
  * @param  huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
...
/**
  ...
  *        IrqHandler API for reception to CHAR_MATCH, IDLE, RTO modes.
  *        (This callback is executed by the IRQHandler when a CM, RTO or IDLE IT is received and DMA transfer
  *        not complete).
  * @param hdma Pointer to a hal_dma_handle_t structure which contains a DMA instance.
  */
static void UART_DMAAbortOnSuccessCallback(hal_dma_handle_t *hdma)
