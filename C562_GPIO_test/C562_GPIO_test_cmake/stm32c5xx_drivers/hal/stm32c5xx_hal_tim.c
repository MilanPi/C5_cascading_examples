/**
  ...
  *           + TIM Break functions
  *           + TIM Deadtime functions
  *           + TIM Protection
  *           + TIM Commutation feature control
  *           + TIM Software Event Generation
  *           + TIM IRQ Handler and Callbacks functions
  *           + TIM Setter and Getter of the user data
  **********************************************************************************************************************
  * @attention
  *
...
/** @addtogroup TIM
  *  @{
  ...
  The compilation flag @ref USE_HAL_TIM_REGISTER_CALLBACKS when set to 1
  allows the user to configure dynamically the driver callbacks.

  Use Function HAL_TIM_Register<Callback_name>() to register a callback.

  It takes as parameters the HAL peripheral handle, and a pointer to the
  ...
  By default, after the Init and when the state is HAL_TIM_STATE_INIT
  all interrupt callbacks are set to the corresponding weak functions HAL_TIM_<Callback_name>().

  Callbacks can be registered in @ref HAL_TIM_STATE_IDLE state only.

  When the compilation flag @ref USE_HAL_TIM_REGISTER_CALLBACKS is set to 0 or
  ...
  USE_HAL_TIM_USER_DATA          | hal_conf.h       | 0                 | Add pointer for user data in HAL TIM handle
...
/* LUT to associate a DMA request and ID for a TIM channel */
...
/* LL TIM Channels lookup table (indexes are given by hal_tim_channel_t) */
...
#if defined(USE_HAL_TIM_DMA) && (USE_HAL_TIM_DMA == 1)
/**
  * @brief Index for tim_dma_config_t table in TIM_Start_DMA_Opt().
  */
#define UPDATE_DMA_REQ_IDX       (0U)
#define COMMUTATION_DMA_REQ_IDX  (1U)
#define TRIGGER_DMA_REQ_IDX      (2U)

/* Define the shift to switch from dma burst source to dma index */
#define TIM_DMABURST_DMAINDEX_SHIFT  (TIM_DCR_DBSS_Pos)
#endif /* USE_HAL_TIM_DMA */

/* Define the shift to switch from LL to HAL constants and vice versa */
...
/* Define the shift to switch from break polarity to break2 polarity */
#define TIM_BREAK2_POLARITY_SHIFT  (12U)  /* Bit position (LL_TIM_BREAK2_POLARITY_HIGH - LL_TIM_BREAK_POLARITY_HIGH) */

/* Mask for the breaks af mode */
#define TIM_BRK2_BRK_MODE_MASK  (0x30000000U)  /* Bit position (TIMx_BDTR.BKBID & TIMx_BDTR.BK2BID) */

/* Mask for the dithering bits in the ARR register */
#define TIM_DITHERING_MASK  (0xFU)  /* Bit position (TIMx_ARR[3:0] & TIMx_CCRy[3:0]) */

/* All optional interrupts mask */
...
/* Mask for all common break input sources. */
#if defined(COMP2)
#define TIM_BREAK_INPUT_SOURCE_MASK \
  (HAL_TIM_BREAK_INPUT_GPIO | HAL_TIM_BREAK_INPUT_COMP1 | HAL_TIM_BREAK_INPUT_COMP2)
#else
#define TIM_BREAK_INPUT_SOURCE_MASK \
  (HAL_TIM_BREAK_INPUT_GPIO | HAL_TIM_BREAK_INPUT_COMP1)
#endif /* COMP2 */

/* Mask for specific TIM1 break input sources. */
#if defined(TIM16)
#define TIM1_BREAK_INPUT_SOURCE_MASK \
  (TIM_BREAK_INPUT_SOURCE_MASK \
   | (HAL_TIM_BREAK_INPUT_TIM8_BKIN1 | HAL_TIM_BREAK2_INPUT_TIM8_BKIN2 \
      | HAL_TIM_BREAK_INPUT_TIM15_BKIN1 | HAL_TIM_BREAK_INPUT_TIM16_BKIN1 \
      | HAL_TIM_BREAK_INPUT_TIM17_BKIN1))
#else
#define TIM1_BREAK_INPUT_SOURCE_MASK \
  (TIM_BREAK_INPUT_SOURCE_MASK \
   | (HAL_TIM_BREAK_INPUT_TIM8_BKIN1 | HAL_TIM_BREAK2_INPUT_TIM8_BKIN2 \
      | HAL_TIM_BREAK_INPUT_TIM15_BKIN1))
#endif /* TIM16 */

/* Mask for specific TIM8 break input sources. */
#if defined(TIM16)
#define TIM8_BREAK_INPUT_SOURCE_MASK \
  (TIM_BREAK_INPUT_SOURCE_MASK \
   | (HAL_TIM_BREAK_INPUT_TIM1_BKIN1 | HAL_TIM_BREAK2_INPUT_TIM1_BKIN2 \
      | HAL_TIM_BREAK_INPUT_TIM15_BKIN1 | HAL_TIM_BREAK_INPUT_TIM16_BKIN1 \
      | HAL_TIM_BREAK_INPUT_TIM17_BKIN1))
#else
#define TIM8_BREAK_INPUT_SOURCE_MASK \
...
/* Mask for specific TIM15 break input sources. */
#if defined(TIM16)
#define TIM15_BREAK_INPUT_SOURCE_MASK \
  (TIM_BREAK_INPUT_SOURCE_MASK \
   | (HAL_TIM_BREAK_INPUT_TIM1_BKIN1 | HAL_TIM_BREAK_INPUT_TIM8_BKIN1 \
      | HAL_TIM_BREAK_INPUT_TIM16_BKIN1 | HAL_TIM_BREAK_INPUT_TIM17_BKIN1))
#else
#define TIM15_BREAK_INPUT_SOURCE_MASK \
  (TIM_BREAK_INPUT_SOURCE_MASK \
   | (HAL_TIM_BREAK_INPUT_TIM1_BKIN1 | HAL_TIM_BREAK_INPUT_TIM8_BKIN1))
...
#if defined(TIM16)
/* Mask for specific TIM16 break input sources. */
#define TIM16_BREAK_INPUT_SOURCE_MASK \
  (TIM_BREAK_INPUT_SOURCE_MASK \
   | (HAL_TIM_BREAK_INPUT_TIM1_BKIN1 | HAL_TIM_BREAK_INPUT_TIM8_BKIN1 \
      | HAL_TIM_BREAK_INPUT_TIM15_BKIN1 | HAL_TIM_BREAK_INPUT_TIM17_BKIN1))

/* Mask for specific TIM17 break input sources. */
#define TIM17_BREAK_INPUT_SOURCE_MASK \
  (TIM_BREAK_INPUT_SOURCE_MASK \
   | (HAL_TIM_BREAK_INPUT_TIM1_BKIN1 | HAL_TIM_BREAK_INPUT_TIM8_BKIN1 \
      | HAL_TIM_BREAK_INPUT_TIM15_BKIN1 | HAL_TIM_BREAK_INPUT_TIM16_BKIN1))
#endif /* TIM16 */
...
/**
  * @brief  Macro for the control of TIM optional interrupts validity (subset of TIM_optional_interruptions).
...
/**
  ...
  * @note   The upper bytes store the LL active input, the lower bytes the LL source polarity.
...
/**
  * @brief  Check the validity of the clock division.
  * @param  div  The clock division to check (@ref hal_tim_clk_division_t).
  * @retval SET (clock division is valid) or RESET (clock division is invalid).
  */
#define IS_TIM_CLK_DIVISION(div) (((div) == HAL_TIM_CLK_DIVISION_DIV1) \
                                  ||((div) == HAL_TIM_CLK_DIVISION_DIV2) \
                                  ||((div) == HAL_TIM_CLK_DIVISION_DIV4) \
                                  ||((div) == HAL_TIM_CLK_DIVISION_DIV8))

/**
  * @brief  Check the validity of the clakc division 2.
  * @param  div  The clock division 2 to check (@ref hal_tim_clk_division2_t).
  * @retval SET (clock division 2 is valid) or RESET (clock division 2 is invalid).
  */
#define IS_TIM_CLK_DIVISION2(div) (((div) == HAL_TIM_CLK_DIVISION2_DIV1)     \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV4)     \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV16)    \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV64)    \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV256)   \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV1024)  \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV4096)  \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV16384) \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV65536) \
                                   ||((div) == HAL_TIM_CLK_DIVISION2_DIV262144))
...
/**
  ...
  *         must preferably not be a pulse.
...
/**
  * @brief  Check the value to store in the Capture/Compare Register (CCRx) when dithering is disabled.
  * @param  instance  TIM instance.
  * @param  pulse     Pulse value.
  * @retval SET (pulse value is valid) or RESET (pulse value is invalid).
  */
#define IS_TIM_OC_PULSE_WITHOUT_DITHERING(instance, pulse) \
  ((IS_TIM_32B_COUNTER_INSTANCE((instance)) == 0U) ?  \
   ((pulse) <= 0x0000FFFFU) : 1U)
...
/**
  ...
  *         solely available for channel unit 3 and 4.
...
/**
  * @brief  Check the validity of the XOR gate input inversion.
  * @param  inv  The XOR gate input inversion to check (@ref hal_tim_ic_xor_gate_input_inversion_t).
  * @retval SET (XOR gate input inversion is valid) or RESET (XOR gate input inversion is invalid).
  */
#define IS_TIM_XOR_GATE_IN_INVERSION(inv) (((inv) == HAL_TIM_IC_XOR_GATE_IN_NON_INVERTED) \
                                           ||((inv) == HAL_TIM_IC_XOR_GATE_IN_INVERTED))
...
/**
  * @brief  Check the validity of the external trigger source.
  * @if IS_TIM_EXT_TRIG_SRC
  * @param  instance  TIM instance.
  * @endif
  * @param  src       The external trigger source to check (@ref hal_tim_ext_trig_src_t).
  * @retval SET (external trigger source is valid) or RESET (external trigger source is invalid).
  */
#if defined(COMP2)
#define IS_TIM1_EXT_TRG_SRC(src) \
  (((src) == HAL_TIM_EXT_TRIG_TIM1_GPIO) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM1_COMP1) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM1_COMP2) \
...
#define IS_TIM2_EXT_TRG_SRC(src) \
  (((src) == HAL_TIM_EXT_TRIG_TIM2_GPIO) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_COMP1) \
   ...
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_ETH_PPS))
#elif defined(TIM5)
#define IS_TIM2_EXT_TRG_SRC(src) \
  (((src) == HAL_TIM_EXT_TRIG_TIM2_GPIO) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_COMP1) \
...
#define IS_TIM2_EXT_TRG_SRC(src) \
  (((src) == HAL_TIM_EXT_TRIG_TIM2_GPIO) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_COMP1) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_COMP2) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_ADC1_AWD1) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_ADC1_AWD2) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_ADC1_AWD3) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM2_LSE) \
...
#define IS_TIM5_EXT_TRG_SRC(src) \
  (((src) == HAL_TIM_EXT_TRIG_TIM5_GPIO) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM5_COMP1) \
...
#define IS_TIM8_EXT_TRG_SRC(src) \
  (((src) == HAL_TIM_EXT_TRIG_TIM8_GPIO) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM8_COMP1) \
...
#define IS_TIM8_EXT_TRG_SRC(src) \
  (((src) == HAL_TIM_EXT_TRIG_TIM8_GPIO) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM8_COMP1) \
   || ((src) == HAL_TIM_EXT_TRIG_TIM8_COMP2) \
...
/**
  * @brief  Check the validity of the channel source.
  * @if IS_TIM_CHANNEL_SRC
  * @param  instance  TIM instance.
  * @endif
  * @param  channel   The channel to check (@ref hal_tim_channel_t).
...
#define IS_TIM1_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM1_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM1_TI1_COMP1) \
        || ((src) == HAL_TIM_INPUT_TIM1_TI1_COMP2))) \
   || (((channel) == HAL_TIM_CHANNEL_2) \
...
#define IS_TIM2_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM2_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_COMP1) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_ETH_PPS) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_LSI) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_LSE) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_RTC_WKUP) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_TIM5_CH1) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_FDCAN1_RXEOF))) \
   || (((channel) == HAL_TIM_CHANNEL_2) \
       && (((src) == HAL_TIM_INPUT_TIM2_TI2_GPIO) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI2_HSE_1MHZ) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI2_MCO1) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI2_MCO2) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI2_FDCAN1_TXEOF))) \
   || (((channel) == HAL_TIM_CHANNEL_3) \
       && (((src) == HAL_TIM_INPUT_TIM2_TI3_GPIO) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI3_FDCAN2_RXEOF))) \
   || (((channel) == HAL_TIM_CHANNEL_4) \
       && (((src) == HAL_TIM_INPUT_TIM2_TI4_GPIO) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI4_COMP1) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI4_FDCAN2_TXEOF))))
#elif defined(TIM5)
#define IS_TIM2_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM2_TI1_GPIO) \
...
#define IS_TIM2_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM2_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_COMP1) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_COMP2) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_LSI) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_LSE) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_RTC_WKUP) \
        || ((src) == HAL_TIM_INPUT_TIM2_TI1_TIM5_CH1) \
   ...
   || (((channel) == HAL_TIM_CHANNEL_4) \
       && (((src) == HAL_TIM_INPUT_TIM2_TI4_GPIO) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI4_COMP1) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI4_COMP2) \
           || ((src) == HAL_TIM_INPUT_TIM2_TI4_FDCAN2_TXEOF))))
#endif /* TIM5 */
...
#define IS_TIM3_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM3_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM3_TI1_COMP1) \
        || ((src) == HAL_TIM_INPUT_TIM3_TI1_ETH_PPS) \
        || ((src) == HAL_TIM_INPUT_TIM3_TI1_FDCAN2_RXEOF))) \
   || (((channel) == HAL_TIM_CHANNEL_2) \
       && (((src) == HAL_TIM_INPUT_TIM3_TI2_GPIO) \
           || ((src) == HAL_TIM_INPUT_TIM3_TI2_FDCAN2_TXEOF))) \
   || (((channel) == HAL_TIM_CHANNEL_3) \
...
#define IS_TIM4_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM4_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM4_TI1_COMP1))) \
   || (((channel) == HAL_TIM_CHANNEL_2) \
...
#define IS_TIM5_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM5_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM5_TI1_COMP1))) \
   || (((channel) == HAL_TIM_CHANNEL_2) \
...
#define IS_TIM8_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM8_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM8_TI1_COMP1) \
        || ((src) == HAL_TIM_INPUT_TIM8_TI1_COMP2))) \
   || (((channel) == HAL_TIM_CHANNEL_2) \
...
#define IS_TIM12_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM12_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM12_TI1_COMP1) \
        || ((src) == HAL_TIM_INPUT_TIM12_TI1_COMP2) \
        || ((src) == HAL_TIM_INPUT_TIM12_TI1_MCO1) \
        || ((src) == HAL_TIM_INPUT_TIM12_TI1_MCO2) \
        || ((src) == HAL_TIM_INPUT_TIM12_TI1_HSE_1MHZ) \
        || ((src) == HAL_TIM_INPUT_TIM12_TI1_I3C1_IBIACK))) \
   || (((channel) == HAL_TIM_CHANNEL_2) \
...
#define IS_TIM15_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM15_TI1_GPIO) \
        || ((src) == HAL_TIM_INPUT_TIM15_TI1_COMP1) \
        || ((src) == HAL_TIM_INPUT_TIM15_TI1_COMP2) \
        || ((src) == HAL_TIM_INPUT_TIM15_TI1_LSE) \
        || ((src) == HAL_TIM_INPUT_TIM15_TI1_FDCAN2_RXEOF))) \
   || (((channel) == HAL_TIM_CHANNEL_2) \
       && ((src) == HAL_TIM_INPUT_TIM15_TI2_GPIO) \
       || ((src) == HAL_TIM_INPUT_TIM15_TI2_FDCAN2_TXEOF)))
#elif defined(FDCAN2)
#define IS_TIM15_CHANNEL_SRC(channel, src) \
  ((((channel) == HAL_TIM_CHANNEL_1) \
    && (((src) == HAL_TIM_INPUT_TIM15_TI1_GPIO) \
...
#define IS_TIM16_CHANNEL_SRC(channel, src) \
  (((channel) == HAL_TIM_CHANNEL_1) \
   && (((src) == HAL_TIM_INPUT_TIM16_TI1_GPIO) \
       || ((src) == HAL_TIM_INPUT_TIM16_TI1_COMP1) \
       || ((src) == HAL_TIM_INPUT_TIM16_TI1_LSI) \
       || ((src) == HAL_TIM_INPUT_TIM16_TI1_LSE) \
       || ((src) == HAL_TIM_INPUT_TIM16_TI1_RTC_WKUP) \
...
#define IS_TIM17_CHANNEL_SRC(channel, src) \
  (((channel) == HAL_TIM_CHANNEL_1) \
   && (((src) == HAL_TIM_INPUT_TIM17_TI1_GPIO) \
       || ((src) == HAL_TIM_INPUT_TIM17_TI1_COMP1) \
       || ((src) == HAL_TIM_INPUT_TIM17_TI1_HSE_1MHZ) \
       || ((src) == HAL_TIM_INPUT_TIM17_TI1_MCO1) \
       || ((src) == HAL_TIM_INPUT_TIM17_TI1_MCO2) \
       || ((src) == HAL_TIM_INPUT_TIM17_TI1_I3C1_IBIACK)))
#endif /* TIM16 */
...
/**
  * @brief  Check the validity of the OCref clear source.
  * @param  src  The OCref clear source to check (@ref hal_tim_ocrefclear_src_t).
  * @retval SET (OCref clear source is valid) or RESET (OCref clear source is invalid).
  */
#if defined(COMP2)
#define IS_TIM_OCREFCLEAR_SRC(src) (((src) == HAL_TIM_OCREFCLEAR_ETR) \
                                    || ((src) == HAL_TIM_OCREFCLEAR_COMP1) \
                                    || ((src) == HAL_TIM_OCREFCLEAR_COMP2))
#else
#define IS_TIM_OCREFCLEAR_SRC(src) (((src) == HAL_TIM_OCREFCLEAR_ETR) \
                                    || ((src) == HAL_TIM_OCREFCLEAR_COMP1))
#endif /* COMP2 */
...
/**
  ...
  */
#if defined(TIM16)
#define IS_TIM_BREAK_INPUT_ALL_SRC(instance, brkinsrc) \
  (((brkinsrc) != 0U) \
   && ((((brkinsrc) & ~(TIM_BREAK_INPUT_SOURCE_MASK)) == 0U) \
       || (((instance) == TIM1) && (((brkinsrc) & ~(TIM1_BREAK_INPUT_SOURCE_MASK)) == 0U)) \
       || (((instance) == TIM8) && (((brkinsrc) & ~(TIM8_BREAK_INPUT_SOURCE_MASK)) == 0U)) \
       || (((instance) == TIM15) && (((brkinsrc) & ~(TIM15_BREAK_INPUT_SOURCE_MASK)) == 0U)) \
       || (((instance) == TIM16) && (((brkinsrc) & ~(TIM16_BREAK_INPUT_SOURCE_MASK)) == 0U)) \
       || (((instance) == TIM17) && (((brkinsrc) & ~(TIM17_BREAK_INPUT_SOURCE_MASK)) == 0U))))
#else
#define IS_TIM_BREAK_INPUT_ALL_SRC(instance, brkinsrc) \
  (((brkinsrc) != 0U) \
   && ((((brkinsrc) & ~(TIM_BREAK_INPUT_SOURCE_MASK)) == 0U) \
...
/**
  ...
  */
#define IS_TIM_BREAK_INPUT_SRC(instance, brkinsrc) \
  ((((brkinsrc) & ((brkinsrc) - 1U)) == 0U) \
   && IS_TIM_BREAK_INPUT_ALL_SRC((instance), (brkinsrc)))
...
#define IS_TIM_BREAK_INPUT_SRC_POLARITY(brkinsrc, polarity) \
  ((((brkinsrc) == HAL_TIM_BREAK_INPUT_GPIO) \
    || ((brkinsrc) == HAL_TIM_BREAK_INPUT_COMP1) \
    || ((brkinsrc) == HAL_TIM_BREAK_INPUT_COMP2)) \
...
/**
  * @brief  Check the validity of the break mode for delayed break mode.
  * @param  brk_mode  The break mode to check (@ref hal_tim_oc_break_mode_t).
  * @retval SET (break mode is valid) or RESET (break mode is invalid).
  */
#define IS_TIM_DELAYED_BREAK_MODE(brk_mode) (IS_TIM_OC_BREAK_MODE(brk_mode) \
                                             &!((brk_mode) == HAL_TIM_OC_BREAKMODE_IMMEDIATE))

/**
  * @brief  Check the validity of the break delay value.
  * @param  break_delay  The break delay value to check.
  * @retval SET (break delay value is valid) or RESET (break delay value is invalid).
  */
#define IS_TIM_BREAK_DELAY(break_delay) ((break_delay) <= 0xFFU)
...
/**
  ...
  * @note   This function calls LL_TIM_SetRemap() which rewrite completely the content
...
/**
  * @brief  DMA transfer stopped callback when triggered by a timer update event.
  * @param  hdma  Pointer to the DMA handle.
  */
...
/**
  ...
  * @param  hdma  Pointer to the DMA handle.
  */
static void TIM_DMAAbortCallback(hal_dma_handle_t *hdma)
{
                               ...
                               const hal_tim_config_t *p_config)
{
  ...
  if (IS_TIM_CLOCK_DIVISION_INSTANCE(p_tim))
  {
    ASSERT_DBG_PARAM(IS_TIM_CLK_DIVISION(p_config->clock_division));
    LL_TIM_SetClockDivision(p_tim, (uint32_t)(p_config->clock_division));
    ASSERT_DBG_PARAM(IS_TIM_CLK_DIVISION2(p_config->clock_division2));
    LL_TIM_SetClockDivision2(p_tim, (uint32_t)(p_config->clock_division2));
  }
                       ...
                       hal_tim_config_t *p_config)
{
  ...
  if (IS_TIM_CLOCK_DIVISION_INSTANCE(p_tim))
  {
    p_config->clock_division = (hal_tim_clk_division_t)LL_TIM_GetClockDivision(p_tim);
    p_config->clock_division2 = (hal_tim_clk_division2_t)LL_TIM_GetClockDivision2(p_tim);
  }
...
/**
  * @brief  Set the clock prescaler division factor (CKD).
  * @param  htim  Pointer to the handle of the TIM instance.
  * @param  clock_division  The clock division.
  * @note   The prescaler set the division ratio between the timer kernel clock (tim_ker_ck)
  *         and the DTS sampling clock (DTS_ck).
  * @note   The DTS sampling clock is used by the dead-time generator
  *         (when supported) and the break/break2 filters.
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_SetClockDivision(hal_tim_handle_t *htim,
                                      hal_tim_clk_division_t clock_division)
{
  ...
  ASSERT_DBG_PARAM(IS_TIM_CLK_DIVISION(clock_division));

  LL_TIM_SetClockDivision(p_tim, (uint32_t)clock_division);

  return HAL_OK;
}

/**
  * @brief  Get the clock prescaler division factor (CKD).
  * @param  htim  Pointer to the handle of the TIM instance.
  * @retval hal_tim_clk_division_t  Clock division for DTS sampling clock.
  */
hal_tim_clk_division_t HAL_TIM_GetClockDivision(const hal_tim_handle_t *htim)
{
  ...
  return (hal_tim_clk_division_t)LL_TIM_GetClockDivision(p_tim);
}

/**
  * @brief  Set the clock prescaler division 2 factor (CKD2).
  * @param  htim  Pointer to the handle of the TIM instance.
  * @param  clock_division2  The clock division 2.
  * @note   The prescaler set the division ratio between the DTS sampling clock (DTS_ck)
  *         and the DTS2 sampling clock (DTS2_ck).
  * @note   The DTS2 sampling clock is by the digital filters.
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_SetClockDivision2(hal_tim_handle_t *htim,
                                       hal_tim_clk_division2_t clock_division2)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  /* Check clock division validity */
  ASSERT_DBG_PARAM(IS_TIM_CLK_DIVISION2(clock_division2));

  LL_TIM_SetClockDivision2(p_tim, (uint32_t)clock_division2);

  return HAL_OK;
}

/**
  * @brief  Get the clock prescaler division 2 factor (CKD2).
  * @param  htim  Pointer to the handle of the TIM instance.
  * @retval hal_tim_clk_division2_t  Clock division 2 for DTS2 sampling clock.
  */
hal_tim_clk_division2_t HAL_TIM_GetClockDivision2(const hal_tim_handle_t *htim)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  return (hal_tim_clk_division2_t)LL_TIM_GetClockDivision2(p_tim);
}
...
/**
  ...
  *          bit 31 of the timer counter register is read-only. This might affect the
  *          counter range in case of 32-bit counter TIM instance.
  * @retval  HAL_OK
  */
...
/**
  ...
  *          4. Capture/compare interrupt flags must be cleared @ref LL_TIM_ClearFlag_CC1 (for each channel)
...
/**
  * @brief  Start the timer in interrupt mode (default TIM interrupts).
  * @param  htim  Pointer to the handle of the TIM instance.
  * @note   The timer is started in interrupt mode with the default interrupts
  *         @arg update by default
  *         @arg commutation if capture/compare preloaded
  *         @arg trigger if slave mode enabled and commutation disabled
  *              or commutation software only
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_Start_IT(hal_tim_handle_t *htim)
{
  ...
  uint32_t is_slave_instance = (IS_TIM_SLAVE_INSTANCE(p_tim)) ? 1U : 0U;
  uint32_t is_slave_mode_enabled = (IS_TIM_SLAVE_MODE_ENABLED(p_tim)) ? 1U : 0U;
  uint32_t trigger_it_activable = 1U; /* Activable by default */

  /* Prepare to enable update interrupts */
  uint32_t it_mask = LL_TIM_DIER_UIE;

  /* Check if either the commutation interrupt or (exclusive)
     the trigger interrupt must be enabled */
  if (IS_TIM_COMMUTATION_EVENT_INSTANCE(p_tim))
  {
    uint32_t is_preload_enabled = LL_TIM_CC_IsEnabledPreload(p_tim);
    if (is_preload_enabled != 0U)
    {
      /* Prepare to enable commutation interrupts */
      it_mask |= LL_TIM_DIER_COMIE;

      /* Trigger interrupt activable only if commutation selection different
         from HAL_TIM_COMMUTATION_SOFTWARE_AND_TRIGGER */
      hal_tim_commutation_src_t com_selection = (hal_tim_commutation_src_t)LL_TIM_CC_GetUpdate(p_tim);
      trigger_it_activable = (com_selection == HAL_TIM_COMMUTATION_SOFTWARE) ? 1U : 0U;
    }
  }

  /* Check if the trigger interrupt must be enabled */
  if ((trigger_it_activable != 0U) && (is_slave_instance != 0U) && (is_slave_mode_enabled != 0U))
  {
    /* Prepare to enable trigger interrupts */
    it_mask |= LL_TIM_DIER_TIE;
  }

  /* Enable interrupts */
  LL_TIM_EnableIT(p_tim, it_mask);
...
hal_status_t HAL_TIM_Start_IT_Opt(hal_tim_handle_t *htim, uint32_t interrupts)
{
  ...
  ASSERT_DBG_PARAM(IS_TIM_OPTIONAL_INTERRUPTS(p_tim, interrupts));
...
/**
  * @brief  Configure the output channel override state.
  * @param  htim            Pointer to the handle of the TIM instance.
  * @param  channel         Output channel to configure.
  * @param  override_state  Output channel override state.
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_OC_SetOverrideState(hal_tim_handle_t *htim,
                                         hal_tim_channel_t channel,
                                         hal_tim_oc_override_state_t override_state)
{
  ...
  /* Check channel configuration parameters */
  ASSERT_DBG_PARAM(IS_TIM_OC_OVERRIDE_STATE(override_state));

  ASSERT_DBG_STATE(htim->channel_states[channel],
                   (HAL_TIM_CHANNEL_STATE_RESET | TIM_CHANNEL_STATE_IDLE));

  LL_TIM_OC_SetOverrideState(p_tim, ll_tim_channels[channel], (uint32_t)override_state);

  return HAL_OK;
}

/**
  * @brief  Get the override state of an output channel.
  * @param  htim     Pointer to the handle of the TIM instance.
  * @param  channel  Output channel.
  * @retval hal_tim_oc_override_state_t  The override state for the output channel.
  */
hal_tim_oc_override_state_t HAL_TIM_OC_GetOverrideState(const hal_tim_handle_t *htim,
                                                        hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  return (hal_tim_oc_override_state_t)LL_TIM_OC_GetOverrideState(p_tim, ll_tim_channels[channel]);
}
...
/**
  ...
  * @param  htim  Pointer to the handle of the TIM instance.
  * @retval hal_tim_oc_override_status_t  Output override status.
  */
hal_tim_oc_override_status_t HAL_TIM_OC_IsEnabledOutputOverride(const hal_tim_handle_t *htim)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  return (hal_tim_oc_override_status_t)LL_TIM_OC_IsEnabledOutputOverride(p_tim);
}

/**
  * @brief  Configure the output channel break mode.
  * @param  htim        Pointer to the handle of the TIM instance.
  * @param  channel     Output channel to configure.
  * @param  break_mode  Output channel break mode.
  * @note   The BKMx[1:0] and BKMxN[1:0] bits are static, they must
  *         only be modified when the counter is stopped (CEN = 0).
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_OC_SetBreakMode(hal_tim_handle_t *htim,
                                     hal_tim_channel_t channel,
                                     hal_tim_oc_break_mode_t break_mode)
{
  ...
  /* Check channel configuration parameters */
  ASSERT_DBG_PARAM(IS_TIM_OC_BREAK_MODE(break_mode));

  ASSERT_DBG_STATE(htim->channel_states[channel],
                   (HAL_TIM_CHANNEL_STATE_RESET | TIM_CHANNEL_STATE_IDLE));

  LL_TIM_OC_SetBreakMode(p_tim, ll_tim_channels[channel], (uint32_t)break_mode);

  return HAL_OK;
}

/**
  * @brief  Get the break mode configuration of an output channel.
  * @param  htim     Pointer to the handle of the TIM instance.
  * @param  channel  Output channel.
  * @retval hal_tim_oc_break_mode_t  The break mode for the output channel.
  */
hal_tim_oc_break_mode_t HAL_TIM_OC_GetBreakMode(const hal_tim_handle_t *htim,
                                                hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  return (hal_tim_oc_break_mode_t)LL_TIM_OC_GetBreakMode(p_tim, ll_tim_channels[channel]);
}
...
/**
  ...
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_OC_EnableChannelComparePreload(hal_tim_handle_t *htim,
                                                    hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  LL_TIM_OC_EnablePreload(p_tim, ll_tim_channels[channel]);
...
/**
  ...
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_OC_DisableChannelComparePreload(hal_tim_handle_t *htim,
                                                     hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  LL_TIM_OC_DisablePreload(p_tim, ll_tim_channels[channel]);
...
/**
  ...
  */
hal_tim_oc_compare_preload_status_t HAL_TIM_OC_IsEnabledChannelComparePreload(const hal_tim_handle_t *htim,
                                                                              hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
...
/**
  ...
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_OC_EnableChannelFastMode(hal_tim_handle_t *htim,
                                              hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  LL_TIM_OC_EnableFast(p_tim, ll_tim_channels[channel]);
...
/**
  ...
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_OC_DisableChannelFastMode(hal_tim_handle_t *htim,
                                               hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  LL_TIM_OC_DisableFast(p_tim, ll_tim_channels[channel]);
...
/**
  ...
  * @param  htim     Pointer to the handle of the TIM instance.
  * @param  channel  Output channel of interest.
  * @retval hal_tim_oc_channel_fast_mode_status_t  Fast Mode status.
  */
hal_tim_oc_channel_fast_mode_status_t HAL_TIM_OC_IsEnabledChannelFastMode(const hal_tim_handle_t *htim,
                                                                          hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  return (hal_tim_oc_channel_fast_mode_status_t)LL_TIM_OC_IsEnabledFast(p_tim,
                                                                        ll_tim_channels[channel]);
}
...
/**
  ...
  * @warning This function selects the input channel as the source of the associated
  *          capture unit (direct).
  * @retval  HAL_OK
  * @retval  HAL_INVALID_PARAM  Input parameter is invalid
  *                            (only if USE_HAL_CHECK_PARAM == 1)
                                         ...
                                         const hal_tim_ic_channel_config_t *p_config)
{
  ...
  /* Set the default active input */
  LL_TIM_IC_SetActiveInput(p_tim, ll_channel, (uint32_t)HAL_TIM_IC_DIRECT);
                                 ...
                                 hal_tim_ic_channel_config_t *p_config)
{
  ...
  p_config->source = (hal_tim_channel_src_t)LL_TIM_GetRemap(p_tim, ll_channel);
                                                  ...
                                                  hal_tim_channel_t channel)
{
  ...
  return (hal_tim_channel_src_t)LL_TIM_GetRemap(p_tim, ll_tim_channels[channel]);
}
...
/**
  ...
  * @warning When a channel HAL_TIM_CHANNEL_X has to be configured together with
  *          the corresponding capture unit HAL_TIM_IC_CAPTURE_UNIT_X (X = 1,2,3
  *          or 4) then HAL_TIM_IC_SetConfigCaptureUnit(HAL_TIM_IC_CAPTURE_UNIT_X)
  *          must be called after HAL_TIM_IC_SetConfigChannel(HAL_TIM_CHANNEL_X).
  *          Otherwise, the configuration of the active input in
  *          HAL_TIM_IC_SetConfigCaptureUnit() is going to be overwritten by the
  *          default setting (@ref HAL_TIM_IC_DIRECT) in @ref HAL_TIM_IC_SetConfigChannel().
...
/**
  * @brief  Configure the signal inversion of a XOR gate input channel.
  * @param  htim       Pointer to the handle of the TIM instance.
  * @param  channel    Input channel to configure. \n
  *                    Must be one of the following values: \n
  *                    @arg @ref HAL_TIM_CHANNEL_1
  *                    @arg @ref HAL_TIM_CHANNEL_2
  *                    @arg @ref HAL_TIM_CHANNEL_3
  * @param  inversion  Input channel signal XOR gate inversion.
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_IC_SetXORGateInputInversion(hal_tim_handle_t *htim,
                                                 hal_tim_channel_t channel,
                                                 hal_tim_ic_xor_gate_input_inversion_t inversion)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  ASSERT_DBG_PARAM((IS_TIM_IC_CHANNEL(p_tim, channel)) && (channel != HAL_TIM_CHANNEL_4));

  /* Check the XOR gate input inversion is valid */
  ASSERT_DBG_PARAM((IS_TIM_XOR_GATE_IN_INVERSION(inversion)));

  ASSERT_DBG_STATE(htim->channel_states[channel],
                   HAL_TIM_IC_CHANNEL_STATE_IDLE);

  LL_TIM_IC_SetXORGateInputInversion(p_tim, ll_tim_channels[channel], (uint32_t)inversion);

  return HAL_OK;
}

/**
  * @brief  Get the signal inversion status of a XOR gate input channel.
  * @param  htim     Pointer to the handle of the TIM instance.
  * @param  channel  Input channel to configure. \n
  *                  Must be one of the following values: \n
  *                  @arg @ref HAL_TIM_CHANNEL_1
  *                  @arg @ref HAL_TIM_CHANNEL_2
  *                  @arg @ref HAL_TIM_CHANNEL_3
  * @retval hal_tim_ic_xor_gate_input_inversion_t  The XOR gate inversion status for the input channel.
  */
hal_tim_ic_xor_gate_input_inversion_t HAL_TIM_IC_GetXORGateInputInversion(const hal_tim_handle_t *htim,
                                                                          hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  return (hal_tim_ic_xor_gate_input_inversion_t)LL_TIM_IC_GetXORGateInputInversion(p_tim, ll_tim_channels[channel]);
}
...
/**
  * @brief  Indicate the value of input signals (after the digital filtering stage),
  ...
  *                  @arg @ref HAL_TIM_CHANNEL_3
  *                  @arg @ref HAL_TIM_CHANNEL_4
  * @retval hal_tim_ic_signal_status_t  Input channel signal status
  */
hal_tim_ic_signal_status_t HAL_TIM_IC_GetInputStatus(const hal_tim_handle_t *htim,
                                                     hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  return (hal_tim_ic_signal_status_t)LL_TIM_IC_GetInputStatus(p_tim, ll_tim_channels[channel]);
}
...
/**
  ...
  * @note   The postscaler is only applicable when tim_trgo2 transfers a pulse
...
hal_status_t HAL_TIM_SetOCRefClearSource(hal_tim_handle_t *htim,
                                         hal_tim_ocrefclear_src_t source)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  ASSERT_DBG_PARAM((IS_TIM_OCREFCLEAR_SRC(source)));
...
/**
  ...
  * @param  htim  Pointer to the handle of the TIM instance.
  * @retval hal_tim_ocrefclear_src_t  OCRef clear source.
  */
hal_tim_ocrefclear_src_t HAL_TIM_GetOCRefClearSource(const hal_tim_handle_t *htim)
{
  ...
  return (hal_tim_ocrefclear_src_t)LL_TIM_GetOCRefClearInputSource(p_tim);
}
...
/**
  ...
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_EnableChannelOCRefClear(hal_tim_handle_t *htim,
                                             hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  ASSERT_DBG_STATE(htim->channel_states[channel],
                   (HAL_TIM_OC_CHANNEL_STATE_IDLE | HAL_TIM_OC_CHANNEL_STATE_ACTIVE));

  LL_TIM_OC_EnableClear(p_tim, ll_tim_channels[channel]);
...
/**
  ...
  * @retval HAL_OK
  */
hal_status_t HAL_TIM_DisableChannelOCRefClear(hal_tim_handle_t *htim,
                                              hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  LL_TIM_OC_DisableClear(p_tim, ll_tim_channels[channel]);
...
/**
  ...
  *                  @arg @ref HAL_TIM_CHANNEL_6
  *                  @arg @ref HAL_TIM_CHANNEL_7
  * @retval hal_tim_ocrefclear_status_t  OCRefClear status.
  */
hal_tim_ocrefclear_status_t HAL_TIM_IsEnabledChannelOCRefClear(const hal_tim_handle_t *htim,
                                                               hal_tim_channel_t channel)
{
  ASSERT_DBG_PARAM((htim != NULL));
  ...
  return (hal_tim_ocrefclear_status_t)LL_TIM_OC_IsEnabledClear(p_tim,
                                                               ll_tim_channels[channel]);
}
                                   ...
                                   uint32_t size_byte)
{
  ...
  hdma->p_xfer_error_cb = TIM_DMAErrorCallback;
  hdma->p_xfer_abort_cb = TIM_DMAAbortCallback;
...
/**
  ...
  *         can be called to re-enable the main output.
  * @retval HAL_OK
  * @retval HAL_ERROR
  */
                                                  ...
                                                  hal_tim_break_input_src_polarity_t polarity)
{
  ...
  ASSERT_DBG_PARAM((IS_TIM_BREAK_INPUT_ID(brkin)));
  ASSERT_DBG_PARAM((IS_TIM_BREAK_INPUT_SRC(p_tim, brkinsrc)));
  ASSERT_DBG_PARAM((IS_TIM_BREAK_INPUT_SRC_POLARITY(brkinsrc, polarity)));

  /* Check if the instance supports break input */
  ASSERT_DBG_PARAM((IS_TIM_BRKIN_INSTANCE(p_tim, brkin)));
  ASSERT_DBG_PARAM((IS_TIM_BREAKSOURCE_INSTANCE(p_tim)));
                                                                        ...
                                                                        uint32_t brkinsrc)
{
  ...
  /* Check if the instance supports break input and break input source */
  ASSERT_DBG_PARAM((IS_TIM_BRKIN_INSTANCE(p_tim, brkin)));
  ASSERT_DBG_PARAM((IS_TIM_BREAKSOURCE_INSTANCE(p_tim)));
...
/**
  ...
  * @param  htim      Pointer to the handle of the TIM instance.
  * @param  brkin     The break input.
  * @param  brkinsrc  This parameter can be a combination of the following values: \n
  *                   @arg @ref HAL_TIM_BREAK_INPUT_GPIO
  *                   @arg @ref HAL_TIM_BREAK_INPUT_COMP1
  * @if COMP2
  *                   @arg @ref HAL_TIM_BREAK_INPUT_COMP2 (*)
  *
  *                   (*)  Value not defined in all devices. \n
  * @endif
  *
  *         Below description summarizes specific "Timer Instance" and "BREAK(2) input source" possibilities:
  *
  *         TIM1: combination of the following values:
  *            @arg @ref HAL_TIM_BREAK_INPUT_TIM8_BKIN1
  *            @arg @ref HAL_TIM_BREAK_INPUT_TIM15_BKIN1
  * @if TIM16
  *            @arg @ref HAL_TIM_BREAK_INPUT_TIM16_BKIN1 (*)
  *            @arg @ref HAL_TIM_BREAK_INPUT_TIM17_BKIN1 (*)
  * @endif
  *
  *            @arg @ref HAL_TIM_BREAK2_INPUT_TIM8_BKIN2
  *
  *         TIM8: combination of the following values:
  *            @arg @ref HAL_TIM_BREAK_INPUT_TIM1_BKIN1
  *            @arg @ref HAL_TIM_BREAK_INPUT_TIM15_BKIN1
  * @if TIM16
  *            @arg @ref HAL_TIM_BREAK_INPUT_TIM16_BKIN1 (*)
  *            @arg @ref HAL_TIM_BREAK_INPUT_TIM17_BKIN1 (*)
  * @endif
  *
  *            @arg @ref HAL_TIM_BREAK2_INPUT_TIM1_BKIN2
  *
                                             ...
                                             uint32_t brkinsrc)
{
  ...
  ASSERT_DBG_PARAM((IS_TIM_BREAK_INPUT_ID(brkin)));
  ASSERT_DBG_PARAM((IS_TIM_BREAK_INPUT_ALL_SRC(p_tim, brkinsrc)));

  /* Check if the instance supports break input source */
  ASSERT_DBG_PARAM((IS_TIM_BRKIN_INSTANCE(p_tim, brkin)));
  ASSERT_DBG_PARAM((IS_TIM_BREAKSOURCE_INSTANCE(p_tim)));
...
/**
  * @brief  Configure the break delay value for a specific delayed break mode.
  * @param  htim         Pointer to the handle of the TIM instance.
  * @param  break_mode   Output channel break mode. \n
  *                      Must be one of the following values: \n
  *                      @arg @ref HAL_TIM_OC_BREAKMODE_DELAY1
  *                      @arg @ref HAL_TIM_OC_BREAKMODE_DELAY2
  * @param  break_delay  Break delay value (number between 0x00 and 0xFF).
  * @note   Break delay (DBKx[7:0]) preload value isn't taken into account
  *         immediately. \n
  *         It is loaded in the active register at next update event.
  * @retval HAL_OK
  */
                                         ...
                                         uint32_t break_delay)
{
  ...
  /* Check that it is a delayed break mode */
  ASSERT_DBG_PARAM((IS_TIM_DELAYED_BREAK_MODE(break_mode)));

  /* Check the break delay value */
  ASSERT_DBG_PARAM((IS_TIM_BREAK_DELAY(break_delay)));

  LL_TIM_SetBreakDelay(p_tim, (uint32_t)break_mode, break_delay);

  return HAL_OK;
}

/**
  * @brief  Get the break delay value for a specific delayed break mode.
  * @param  htim        Pointer to the handle of the TIM instance.
  * @param  break_mode  Output channel break mode. \n
  *                     Must be one of the following values: \n
  *                     @arg @ref HAL_TIM_OC_BREAKMODE_DELAY1
  *                     @arg @ref HAL_TIM_OC_BREAKMODE_DELAY2
  * @retval uint32_t  Break delay value (number between 0x00 and 0xFF).
  */
                                     ...
                                     hal_tim_oc_break_mode_t break_mode)
{
  ...
  return LL_TIM_GetBreakDelay(p_tim, (uint32_t)break_mode);
}
...
/**
  ...
  * @param  htim  Pointer to the handle of the TIM instance.
  * @return hal_tim_asymmetrical_deadtime_status_t  Asymmetrical deadtime status.
  */
...
void HAL_TIM_IRQHandler(hal_tim_handle_t *htim)
{
  ...
  const uint32_t break_it_source = (uint32_t)IS_BIT_SET(it_sources, LL_TIM_DIER_BIE); /* for break registers bitfields
                                                                                         not aligned */
  ...
  if (IS_BIT_SET(flag_status, LL_TIM_SR_SBIF) && (break_it_source != 0U))
  {
  ...
  if (IS_BIT_SET(flag_status, LL_TIM_SR_BGF) && (break_it_source != 0U))
  {
  ...
  if (IS_BIT_SET(flag_status, LL_TIM_SR_B2GF) && (break_it_source != 0U))
  {
  ...
  if (IS_BIT_SET(flag_status, TIM_SR_B2IF) && (break_it_source != 0U))
  {
...
void HAL_TIM_BRK_TERR_IERR_IRQHandler(hal_tim_handle_t *htim)
{
  ...
  if (IS_BIT_SET(flag_status, LL_TIM_SR_B2IF) && (break_it_source != 0U))
  {
    LL_TIM_ClearFlag_BRK2(p_tim);
