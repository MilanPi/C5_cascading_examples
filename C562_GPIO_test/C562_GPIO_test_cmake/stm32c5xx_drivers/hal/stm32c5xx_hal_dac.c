...
/** @defgroup DAC DAC
  * @{
  ...
  ## DAC channels
    ...
    Up to 2 channels converters (i.e. channel1 and channel2)
    can be used independently (separate mode) or simultaneously (dual mode):
      - DAC channel output can be connected to on chip peripherals or to GPIO for voltage generation on device pin.

  ## DAC triggers

    Digital to analog conversion can be non-triggered using HAL_DAC_TRIGGER_NONE
  ...
  ## DAC output buffer mode feature
    ...
    without having to add an external amplifier.
  ...
  ## Channel in separate mode, input and output operation
    ...
    + Use HAL_DAC_StartChannel() to enable one channel and start conversion
        for this channel.<br>
        Careful: when software trigger is selected, the call of HAL_DAC_StartChannel() starts
        the first conversion with the data previously present in DAC_DHRx register.
        So, a first call to HAL_DAC_SetChannelData is required in order to fix the initial value of DAC_DHRx.
        Then next conversions will use data set with HAL_DAC_SetChannelData().
    ...
    + Use HAL_DAC_StartChannel_DMA() to enable one channel and start conversion
        ...
        Careful: first issued trigger starts the conversion with the data previously present in DAC_DHRx register.
        So, a first call to HAL_DAC_SetChannelData is required in order to fix the initial value of DAC_DHRx.<br>
  ...
  ## DAC sample and hold feature
    ...
    converted voltage on a capacitor. In case of DAC output connected to on chip peripheral, this capacitor is
    internal. In case of DAC output connected to pin, an external capacitor must be connected to the output pin.<br>
    ...
    The sample and hold block and registers uses either LSI and run in
  ...
  ## DAC calibration feature
    ...
    @note On this STM32 series there is only one DAC instance with a single channel.
          All reference to channel 2 are not applicable to this series.
    - The converter (channel1) provide calibration capabilities.
    ...
    - Use HAL_DAC_GetChannelBufferCalibrationValue() to retrieve the trimming value (the trimming factory setting
         after reset, or the user setting if HAL_DAC_SetChannelBufferCalibrationValue() have been used
         at least one time after reset).<br>
         The user might store the calibration results for latter usage (for instance: monitoring the trimming
  ...
  ## DAC data value to voltage correspondence
     ...
     where DOR is the Data Output Register
     where Vref+ is the input voltage reference (refer to the device datasheet)<br>
     e.g. :
     ...
     @note A helper macro is available to calculate the DAC conversion data (unit: digital value) corresponding
           to a voltage (unit: mVolt) refer to LL DAC driver: LL_DAC_CALC_VOLTAGE_TO_DATA

  ## DMA requests

    A DMA request can be generated when an external trigger (but not a software trigger)
    occurs if DMA requests are enabled using HAL_DAC_StartChannel_DMA().

  ## High frequency interface mode

    The high frequency interface informs DAC instance about the bus frequency in use.
    This is a mandatory information for DAC (as internal timing of DAC is bus frequency dependent).
    This is provided with the parameter high_frequency_mode handled in HAL_DAC_SetConfig() function.<br>

    The optimum frequency interface mode for the DAC peripheral can be determined by
    calling HAL_DAC_GetOptimumFrequencyMode().
    Then the optimum high frequency interface mode (HFSEL bits) could be set with HAL_DAC_SetConfig().<br>
...
## The DAC HAL driver can be used as follows (in separate channel configuration):
  ...
  ### Initialize the DAC low level resources :
    + DAC APB clock must be enabled to get write access to DAC.
      ...
      Note: clock is enabled inside HAL_DAC_Init() whenever USE_HAL_DAC_CLK_ENABLE_MODEL is not set
            to HAL_CLK_ENABLE_NO.

    + Declare a hal_dac_handle_t handle structure, for example:  hal_dac_handle_t  hdac;
    + initialize DAC instance using HAL_DAC_Init().
    + Configure the DAC output GPIO pins for the used channels in analog mode.
    + Configure the DAC instance with HAL_DAC_SetConfig() function.
    + Configure the DAC channel with HAL_DAC_SetConfigChannel() function.
    + If needed link DAC with DMA with HAL_DAC_SetChannelDMA().
    + Enable the DAC channel with HAL_DAC_StartChannel() or with HAL_DAC_StartChannel_DMA() functions.
    ...
    ### Polling mode, input and output operation
       ...
       + To read the DAC last data output value, use the HAL_DAC_GetChannelData() function.
    ...
    ### DMA mode, input and output operation
       ...
       + Start the DAC peripheral on a given channel using DMA with HAL_DAC_StartChannel_DMA(),
         the user specifies a data buffer and the data size to be converted through DMA.<br>
         So, a first call to HAL_DAC_SetChannelData is required in order to fix the initial value of DAC_DHRx.
       + At the middle of data transfer HAL_DAC_ConvHalfCpltCallback()
           function is executed (user might have added his own code by overriding this weak function).
       + At the end of data transfer HAL_DAC_ConvCpltCallback() function is executed
           (user might have added his own code by overriding this weak function).
       + In case of transfer error, HAL_DAC_ErrorCallback() function is executed (user might have added his own code
           by overriding this weak function).

       + In case of DMA underrun, DAC interruption triggers and executes internal function HAL_DAC_IRQHandler.
           HAL_DAC_ErrorCallback() function is executed (user might have added his own code
           by overriding this weak function).
    ...
    ### Callback registration
       ...
       + Use Functions HAL_DAC_RegisterConvHalfCpltCallback() to register
         HAL_DAC_ConvHalfCpltCallback() : callback when a half transfer is completed on a channel.
       + Use Functions HAL_DAC_RegisterConvCpltCallback() to register
         HAL_DAC_ConvCpltCallback()     : callback when a transfer is completed on a channel.
       + Use Functions HAL_DAC_RegisterErrorCallback() to register
       ...
       Callbacks can only be registered in
       state HAL_DAC_STATE_SEPARATE_CHANNEL_CONFIGURED.
...
USE_HAL_DAC_USER_DATA          | hal_conf.h         | 0 | enable the user data
USE_HAL_DAC_REGISTER_CALLBACKS | hal_conf.h         | 0 | enable the register callbacks assert
USE_HAL_DAC_CLK_ENABLE_MODEL   | hal_conf.h         | HAL_CLK_ENABLE_NO | else, clock is enabled inside HAL_DAC_Init()
USE_HAL_DAC_DMA                | hal_conf.h         | 1 | to use the DMA with the DAC
USE_HAL_CHECK_PARAM            | hal_conf.h         | 0 | to use the run-time checks on function parameters
USE_HAL_CHECK_PROCESS_STATE    | hal_conf.h         | 0 | to use the run-time checks on the state during processing
...
USE_HAL_DAC_GET_LAST_ERRORS    | hal_conf.h         | 0 | records the errors occurred during DAC channel processing
...
#define IS_DAC_TRIGGER(hdac, trigger) (((trigger) == HAL_DAC_TRIGGER_NONE)          \
                                       ...
                                       || ((trigger) == HAL_DAC_TRIGGER_LPTIM1_CH1) \
...
/*! Old time when "sample and hold mode" is enabled */
...
/*! Lookup table for channel DMA underrun interruption */
...
/**
  ...
  * @note   After calling this function the DAC jump to HAL_DAC_STATE_SEPARATE_CHANNEL_CONFIGURED, and it is possible
...
void HAL_DAC_ResetConfig(hal_dac_handle_t *hdac)
{
  ...
  for (uint32_t index = 0; index < DAC_NB_OF_CHANNEL; index++)
  {
    ...
    reg_value |= ((uint32_t)(LL_DAC_MODE_NORMAL_OPERATION)
                  ...
                  | (uint32_t)(LL_DAC_HIGH_FREQ_MODE_DISABLE)
                 ) << lut_ch_shift[index];
...
hal_status_t HAL_DAC_SetChannelData(hal_dac_handle_t *hdac, hal_dac_channel_t channel, uint32_t data)
{
  ...
  /**
    ...
    * tmp_reg is needed to avoid to change other bits (ex: DAC_DHR12L2 )
...
hal_status_t HAL_DAC_StopChannel_DMA(hal_dac_handle_t *hdac, hal_dac_channel_t channel)
{
  ...
  LL_DAC_DisableIT_DMAUDR(p_instance, LL_DAC_IT_DMAUDRIE1);
...
/**
  * @brief  Check whether the DAC DMA double data mode mode is enabled or not.
  * @param  hdac Pointer to a hal_dac_handle_t.
  * @param  channel The selected DAC channel.
                                                   ...
                                                   const hal_dac_channel_sample_and_hold_config_t *p_config)
{
  ...
  while (LL_DAC_IsActiveFlag_BWST1(p_instance) != 0UL)
  {
    /* Check for the Timeout */
    if ((HAL_GetTick() - tickstart) > DAC_TIMEOUT_FOR_BWST_MS)
    {
      /* New check to avoid false timeout detection in case of preemption */
      if (LL_DAC_IsActiveFlag_BWST1(p_instance) != 0UL)
      {
        return HAL_ERROR;
      }
    }
  ...
  LL_DAC_SetSampleAndHoldSampleTime(p_instance, LL_DAC_CHANNEL_1,
...
/**
  * @brief Check whether the DAC sample and hold mode is enabled or disabled for a channel.
  * @param hdac Pointer to a hal_dac_handle_t.
  * @param channel The selected DAC channel.
                                             ...
                                             uint32_t size_byte, uint32_t dma_opt_interrupt)
{
  ...
  LL_DAC_EnableIT_DMAUDR(p_instance, lut_ch_dma_underrun_it[HAL_DAC_CHANNEL_1]);
