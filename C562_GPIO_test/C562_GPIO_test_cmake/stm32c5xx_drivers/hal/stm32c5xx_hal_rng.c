...
- Configure the RNG peripheral with one of the following configuration :
  ...
  5. Additional Health tests  configuration:
     - Call HAL_RNG_SetHealthFactorConfig() function.
...
/*! Check RNG additional heath test register index */
#define IS_RNG_HTCR_INDEX(idx) \
  (((idx) == HAL_RNG_HTCR1 ) \
   || ((idx) == HAL_RNG_HTCR2 ) \
   || ((idx) == HAL_RNG_HTCR3 ))

/*! Check RNG additional heath test value */
#define IS_RNG_HTCR_VALUE(value) ((value) <= (0xFFFF))
...
hal_status_t HAL_RNG_SetCertifiedNISTConfig(hal_rng_handle_t *hrng)
{
  ...
  if (LL_RNG_IsConfigLocked(p_rngx) == 0U)
  {
    if (RNG_WaitOnBusyFlagUntilTimeout(hrng) == HAL_OK)
    {
      ...
      LL_RNG_WRITE_REG(p_rngx, HTCR[0], RNG_CERT_NIST_HTCR_VALUE);
      LL_RNG_DisableCondReset(p_rngx);
...
/**
  ...
  * @param  hrng      pointer to @ref hal_rng_handle_t structure.
  * @retval HAL_OK    operation completed successfully.
  * @retval HAL_ERROR operation exceeds the busy flag timeout.
  */
static hal_status_t RNG_WaitOnBusyFlagUntilTimeout(hal_rng_handle_t *hrng)
{
  uint32_t tickstart;
  ...
  do
  {
    if (LL_RNG_IsActiveFlag_BUSY(RNG_GET_INSTANCE(hrng)) == 0U)
    {
      return HAL_OK;
    }
  } while ((HAL_GetTick() - tickstart) > RNG_BUSY_TIMEOUT_MS);
