...
/*! Tests calibration subtracted pulses */
...
/** @addtogroup RTC_Exported_Functions_Config
  ...
  * The RTC peripheral has got a programmable prescaler stage which can be configured to generate a 1Hz clock.
  ...
  *
  * The following equation gives us the clock frequency after the first prescaler :
  * freq_async_clk = freq_rtcclk/(1+@ref hal_rtc_config_t::asynch_prediv)
  *
  * The following equation gives us the clock frequency after the two prescalers :
                                           ...
                                           hal_rtc_timestamp_information_t *p_info)
{
  ...
  p_info->flag = (hal_rtc_timestamp_event_flag_t)(LL_RTC_READ_REG(SR) & (LL_RTC_SR_TSF | LL_RTC_SR_TSOVF));
