...
4. Configure and/or enable advanced features. For instance, HAL_SMBUS_EnableAnalogFilter, HAL_SMBUS_SetDigitalFilter,
   HAL_SMBUS_SetConfigOwnAddress2, HAL_SMBUS_EnableOwnAddress2, ...APIs.
...
6. Callbacks definition in Interrupt
  - When the compilation define USE_HAL_SMBUS_REGISTER_CALLBACKS is set to 1U, the user can configure dynamically the
  ...
  When the compilation define USE_HAL_SMBUS_REGISTER_CALLBACKS is set to 0U or not defined, the callback registration
...
7. Acquire/Release the SMBUS bus
  ...
  - When the compilation flag USE_HAL_MUTEX is set to 0 or not defined, HAL_SMBUS_AcquireBus/HAL_SMBUS_ReleaseBus
    are not available.
...
-------------------------------- | -----------------| ----------------- | ---------------------------------------------
USE_HAL_SMBUS_CLK_ENABLE_MODEL   | hal_conf.h       | HAL_CLK_ENABLE_NO | Enable peripheral clock in the HAL_SMBUS_Init
USE_HAL_SMBUS_MODULE             | hal_conf.h       |        1U         | When set, HAL SMBUS module is enabled
USE_HAL_SMBUS_USER_DATA          | hal_conf.h       |        0U         | Enable the set/get user data
USE_HAL_SMBUS_REGISTER_CALLBACKS | hal_conf.h       |        0U         | Enable the register callbacks assert
USE_HAL_CHECK_PARAM              | hal_conf.h       |        0U         | Enable checking of vital parameters at runtime
USE_HAL_CHECK_PROCESS_STATE      | hal_conf.h       |        0U         | Check the HAL state transition in run time
USE_HAL_MUTEX                    | hal_conf.h       |        0U         | Enable the acquire/release bus services
...
#define SMBUS_STATE_SLAVE_BUSY_RX   (1UL << 3U)                 /*!< Memory Busy TX */
...
/**
  ...
  */
#define SMBUS_CHECK_IT_SOURCE(cr1, it)      ((((cr1) & (it)) == (it)) ? SET : RESET)
...
/**
  ...
  * @retval HAL_ERROR HAL SMBUS semaphore creation is failed (USE_HAL_MUTEX is set to 1)
  */
hal_status_t HAL_SMBUS_Init(hal_smbus_handle_t *hsmbus, hal_smbus_t instance)
{
...
#if defined (USE_HAL_SMBUS_REGISTER_CALLBACKS) && (USE_HAL_SMBUS_REGISTER_CALLBACKS == 1U)
  ...
  hsmbus->xfer_opt = (hal_smbus_xfer_opt_t) 0UL;
  hsmbus->xfer_isr = NULL;

  hsmbus->last_error_codes = HAL_SMBUS_ERROR_NONE;

#if defined (USE_HAL_SMBUS_USER_DATA) && (USE_HAL_SMBUS_USER_DATA == 1U)
...
/**
  * @brief  Set hardware timeout config.
  * @param  hsmbus Pointer to a @ref hal_smbus_handle_t
...
/**
  * @brief  Get hardware timeout config.
  * @param  hsmbus Pointer to a @ref hal_smbus_handle_t
  * @param  p_config Pointer to hal_smbus_timeout_config_t containing both TimeoutA and B configuration.
  */
...
hal_smbus_timeout_t HAL_SMBUS_IsEnabledTimeoutA(const hal_smbus_handle_t *hsmbus)
{
  ...
  return (hal_smbus_timeout_t)((uint32_t)READ_BIT(p_i2cx->TIMEOUTR, I2C_TIMEOUTR_TIMOUTEN));
}
...
hal_smbus_timeout_t HAL_SMBUS_IsEnabledTimeoutB(const hal_smbus_handle_t *hsmbus)
{
  ...
  return (hal_smbus_timeout_t)((uint32_t)READ_BIT(p_i2cx->TIMEOUTR, I2C_TIMEOUTR_TEXTEN));
}
...
- There are two modes of transfer :
  ...
  - Non-Blocking mode : The communication is performed using Interrupts as DMA is not supported due to SMBUS need
...
/**
  ...
  */
hal_status_t HAL_SMBUS_MASTER_IsSlaveReady(hal_smbus_handle_t *hsmbus, uint32_t device_addr, uint32_t trials,
                                           uint32_t timeout_ms)
{
  I2C_TypeDef *p_i2cx;
  uint32_t tick_start;
  volatile uint32_t trial_counter = 0UL;
  uint32_t tmp1;
  uint32_t tmp2;
  ...
  do
  {
    ...
    if (LL_I2C_IsActiveFlag_NACK(p_i2cx) == 0U)
    {
      ...
      if (SMBUS_WaitOnFlagUntilTimeout(hsmbus, LL_I2C_ISR_STOPF, (uint32_t)RESET, timeout_ms, tick_start) != HAL_OK)
      {
                                              ...
                                              uint32_t size_byte, hal_smbus_xfer_opt_t xfer_opt)
{
  ...
  ASSERT_DBG_PARAM((p_data != NULL) || (size_byte == 0UL));
  ASSERT_DBG_PARAM(IS_SMBUS_ADDRESS(device_addr));
...
#if defined(USE_HAL_CHECK_PARAM) && (USE_HAL_CHECK_PARAM == 1)
  if ((p_data == NULL) && (size_byte != 0UL)) /* p_data can be NULL if size_byte = 0 */
  {
    return HAL_INVALID_PARAM;
  }
#endif /* USE_HAL_CHECK_PARAM */
                                             ...
                                             uint32_t size_byte, hal_smbus_xfer_opt_t xfer_opt)
{
  ...
  else
  {
    ...
    if ((hsmbus->previous_state == (uint32_t)HAL_SMBUS_STATE_RX)
        && (IS_SMBUS_TRANSFER_OTHER_OPTIONS_REQUEST(tmp) == 0UL))
    {
      request = SMBUS_NO_STARTSTOP;
    }
    /* Else transfer direction change, so generate Restart with new transfer direction */
                                             ...
                                             hal_smbus_xfer_opt_t xfer_opt)
{
  ...
  else
  {
    ...
    /* PEC byte is automatically sent by HW block, no need to manage it in Transmit process */
    if (LL_I2C_IsEnabledSMBusPEC(p_i2cx) != 0UL)
    {
      hsmbus->xfer_size--;
      hsmbus->xfer_count--;
    }
                                            ...
                                            hal_smbus_xfer_opt_t xfer_opt)
{
  ...
  if (((LL_I2C_IsEnabledSMBusPEC(p_i2cx) != 0UL) && (hsmbus->xfer_size == 2U)) || (hsmbus->xfer_size == 1U))
  {
...
/**
  * @brief  Acquire the SMBUS bus thanks to the the HAL OS abstraction layer (stm32_hal_os.c/.h osal).
  * @param  hsmbus Pointer to a @ref hal_smbus_handle_t
  * @param  timeout_ms Timeout duration in millisecond.
  * @note   The HAL_SMBUS_AcquireBus must be called from thread mode only (not from handler mode e.g from ISR).
  * @retval HAL_OK    Operation completed successfully
  * @retval HAL_ERROR Operation completed with error
  */
...
static hal_status_t SMBUS_Master_ISR(hal_smbus_handle_t *hsmbus, uint32_t it_flags, uint32_t it_sources)
{
  ...
  if (SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_NACKF) != RESET)
  {
    LL_I2C_ClearFlag_NACK(p_i2cx);
  ...
  else if (SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_STOPF) != RESET)
  {
    ...
    if (hsmbus->global_state == HAL_SMBUS_STATE_TX)
    {
      ...
      CLEAR_BIT(p_i2cx->CR2, I2C_CR2_SADD | I2C_CR2_HEAD10R | I2C_CR2_NBYTES
    ...
    else if (hsmbus->global_state == HAL_SMBUS_STATE_RX)
    {
      ...
      if ((SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_RXNE) != RESET)
          && (SMBUS_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_RXIE) != RESET))
      {
        /* Read data from RXDR */
  ...
  else if (SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_RXNE) != RESET)
  {
    /* Read data from RXDR */
    *hsmbus->p_buf_rx = LL_I2C_ReceiveData8(p_i2cx);
  ...
  else if (SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_TXIS) != RESET)
  {
    /* Write data to TXDR */
  ...
  else if (SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_TCR) != RESET)
  {
  ...
  else if (SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_TC) != RESET)
  {
...
static hal_status_t SMBUS_Slave_ISR(hal_smbus_handle_t *hsmbus, uint32_t it_flags, uint32_t it_sources)
{
  ...
  else if (SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_ADDR) != RESET)
  {
    ...
    HAL_SMBUS_SLAVE_AddrCallback(hsmbus, TransferDirection, SlaveAddrCode);
           ...
           || (SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_TCR) != RESET))
  {
  ...
  if ((SMBUS_CHECK_FLAG(it_flags, LL_I2C_ISR_STOPF) != RESET)
      && (SMBUS_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_STOPIE) != RESET))
  {
...
static void SMBUS_Disable_IRQ(hal_smbus_handle_t *hsmbus, uint32_t it_request)
{
  ...
  if ((it_request & SMBUS_TX_IT_MASK) == SMBUS_TX_IT_MASK)
  {
    ...
    if ((LL_I2C_IsEnabledSMBusAlert(p_i2cx) != 0UL)
...
static hal_status_t SMBUS_IsErrorOccurred(hal_smbus_handle_t *hsmbus, uint32_t timeout_ms, uint32_t tick_start)
{
  ...
  if (IS_BIT_SET(it_flag, LL_I2C_ISR_NACKF))
  {
    LL_I2C_ClearFlag_NACK(p_i2cx);
    ...
    while ((LL_I2C_IsActiveFlag_STOP(p_i2cx) == 0U) && (status == HAL_OK))
    {
      ...
      if (timeout_ms != HAL_MAX_DELAY)
      {
        if (((HAL_GetTick() - tick_start_local) > timeout_ms) || (timeout_ms == 0U))
        {
          ...
          {
            /* Check for the timeout */
            if ((HAL_GetTick() - tick_start_local) > HAL_TIMEOUT_STOPF)
            {
              status = HAL_ERROR;
              break;
            }
  ...
  if (IS_BIT_SET(it_flag, LL_I2C_ISR_BERR))
  {
  ...
  if (IS_BIT_SET(it_flag, LL_I2C_ISR_OVR))
  {
  ...
  if (IS_BIT_SET(it_flag, LL_I2C_ISR_ARLO))
  {
