...
6. Callback registration
  - When the compilation flag USE_HAL_I2C_REGISTER_CALLBACKS is set to 1, it allows the user to configure dynamically
    the driver callbacks instead of weak functions.
    Functions allows to register callbacks:
      - HAL_I2C_MASTER_RegisterTxCpltCallback() for master transmission end of transfer callback registration.
      - HAL_I2C_MASTER_RegisterRxCpltCallback() for master reception end of transfer callback registration.
      - HAL_I2C_SLAVE_RegisterTxCpltCallback() for slave reception end of transfer callback registration.
      - etc

  - When the compilation flag USE_HAL_I2C_REGISTER_CALLBACKS is set to 0 or not defined, the callback registration
...
7. Acquire/Release the i2c bus
  ...
  - When the compilation flag USE_HAL_MUTEX is set to 0 or not defined, HAL_I2C_AcquireBus/HAL_I2C_ReleaseBus
    are not available.
...
------------------------------ | --------------------- | ------------- | --------------------------------------------
USE_HAL_I2C_CLK_ENABLE_MODEL   | stm32c5xx_hal_conf.h  | HAL_CLK_ENABLE_NO | Enable the gating of the periph clk
USE_HAL_I2C_MODULE             | stm32c5xx_hal_conf.h  | 1  | When set, HAL I2C module is enabled
USE_HAL_I2C_DMA                | stm32c5xx_hal_conf.h  | 1  | Enable DMA code inside I2C
USE_HAL_I2C_GET_LAST_ERRORS    | stm32c5xx_hal_conf.h  | 0  | Enable the get last process error
USE_HAL_I2C_USER_DATA          | stm32c5xx_hal_conf.h  | 0  | Enable the set/get user data
USE_HAL_I2C_REGISTER_CALLBACKS | stm32c5xx_hal_conf.h  | 0  | Enable the register callbacks assert
...
USE_HAL_CHECK_PROCESS_STATE    | stm32c5xx_hal_conf.h  | 0  | Check the HAL I2C state transition in run time
...
#define I2C_STATE_NONE             0         /*!< Default Value */
#define I2C_STATE_MASTER_BUSY_TX  (1UL << 0) /*!< Master Busy TX */
#define I2C_STATE_MASTER_BUSY_RX  (1UL << 1) /*!< Master Busy RX */
#define I2C_STATE_SLAVE_BUSY_TX   (1UL << 2) /*!< Slave Busy TX */
#define I2C_STATE_SLAVE_BUSY_RX   (1UL << 3) /*!< Memory Busy TX */

/**
  * @brief All States listen
  */
#define I2C_STATE_ALL_LISTEN (((uint32_t) HAL_I2C_STATE_LISTEN) \
                              | ((uint32_t) HAL_I2C_STATE_RX_LISTEN) \
                              | ((uint32_t) HAL_I2C_STATE_TX_LISTEN))
/**
  * @brief All States except Init and Idle
  */
#define I2C_STATE_ALL_ACTIVE (((uint32_t) HAL_I2C_STATE_TX) \
                              | ((uint32_t) HAL_I2C_STATE_RX) \
                              | ((uint32_t) I2C_STATE_ALL_LISTEN))
/**
  * @brief All States except Init
  */
#define I2C_STATE_ALL_EXCEPT_INIT (((uint32_t) HAL_I2C_STATE_IDLE) \
                                   | ((uint32_t) I2C_STATE_ALL_ACTIVE) \
                                   | ((uint32_t) HAL_I2C_STATE_ABORT))
/**
  * @brief All States
  */
#define I2C_STATE_ALL (((uint32_t) HAL_I2C_STATE_INIT) \
                       | ((uint32_t) I2C_STATE_ALL_EXCEPT_INIT))
...
/** @defgroup I2C_Interrupt_configuration I2C interrupt configuration for disable
  * @{
  */
#define I2C_XFER_TX_IT      0x0001UL /*!< Bitfield can be combined with @ref I2C_XFER_LISTEN_IT */
#define I2C_XFER_RX_IT      0x0002UL /*!< Bitfield can be combined with @ref I2C_XFER_LISTEN_IT */
#define I2C_XFER_LISTEN_IT  0x8000UL /*!< Bitfield can be combined with @ref I2C_XFER_TX_IT and @ref I2C_XFER_RX_IT */
#define I2C_XFER_ERROR_IT   0x0010UL /*!< Bit definition to manage addition of global Error and NACK treatment */
#define I2C_XFER_CPLT_IT    0x0020UL /*!< Bit definition to manage only STOP event */
#define I2C_XFER_RELOAD_IT  0x0040UL /*!< Bit definition to manage only Reload of NBYTE */
/**
  * @}
  */
...
/* Mask can be combined with @ref I2C_XFER_TX_IT_AMSK and @ref I2C_XFER_RX_IT_MASK */
...
/**
  * @brief Check flag
  */
#define I2C_CHECK_FLAG(isr, flag) ((((isr) & (flag)) == (flag)) ? SET : RESET)

/**
  * @brief Check IT source
  */
#define I2C_CHECK_IT_SOURCE(cr1, it)  ((((cr1) & (it)) == (it)) ? SET : RESET)
...
/* Treat different error callback */
...
/**
  ...
  * @retval HAL_ERROR HAL I2C semaphore creation is failed (USE_HAL_MUTEX is set to 1)
  */
hal_status_t HAL_I2C_Init(hal_i2c_handle_t *hi2c, hal_i2c_t instance)
{
  ...
  hi2c->xfer_size = 0U;
  hi2c->xfer_count = 0U;
  hi2c->xfer_opt = (hal_i2c_xfer_opt_t) 0UL;
  hi2c->xfer_isr = NULL;
...
void HAL_I2C_DeInit(hal_i2c_handle_t *hi2c)
{
  ...
  ASSERT_DBG_PARAM((hi2c != NULL));
  ASSERT_DBG_STATE(hi2c->global_state, I2C_STATE_ALL);
  p_i2cx = I2C_GET_INSTANCE(hi2c);

#if defined (USE_HAL_I2C_USER_DATA) && (USE_HAL_I2C_USER_DATA == 1)
  hi2c->p_user_data = NULL;
#endif /* USE_HAL_I2C_USER_DATA */
...
void HAL_I2C_GetConfig(const hal_i2c_handle_t *hi2c, hal_i2c_config_t *p_config)
{
  ...
  ASSERT_DBG_PARAM((hi2c != NULL));
  ASSERT_DBG_PARAM((p_config != NULL));
  ASSERT_DBG_STATE(hi2c->global_state, I2C_STATE_ALL_EXCEPT_INIT);
  p_i2cx = I2C_GET_INSTANCE(hi2c);
...
/**
  ...
  * @param  hi2c Pointer to a hal_i2c_handle_t
  * @param  value Coefficient of digital noise filter between Min_Data=0x00 and Max_Data=0x0F.
  * @retval HAL_OK Operation completed successfully
  */
hal_status_t HAL_I2C_SetDigitalFilter(hal_i2c_handle_t *hi2c, uint32_t value)
{
  ...
  ASSERT_DBG_PARAM((hi2c != NULL));
  ASSERT_DBG_PARAM(IS_I2C_DIGITAL_FILTER(value));
  ASSERT_DBG_STATE(hi2c->global_state, (uint32_t)HAL_I2C_STATE_IDLE);
  p_i2cx = I2C_GET_INSTANCE(hi2c);

  LL_I2C_Disable(p_i2cx);
  LL_I2C_SetDigitalFilter(p_i2cx, value);
  LL_I2C_Enable(p_i2cx);
                                     ...
                                     uint32_t size_byte, uint32_t timeout_ms)
{
  ...
  hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_BUSY, (uint32_t)SET, I2C_TIMEOUT_BUSY, tick_start);
    ...
    while (hi2c->xfer_count > 0U)
    {
      ...
      if (hal_status == HAL_OK)
      {
        ...
        if ((hi2c->xfer_count != 0U) && (hi2c->xfer_size == 0U))
        {
          ...
          hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_TCR, (uint32_t)RESET, timeout_ms, tick_start);
          if (hal_status == HAL_OK)
          {
            if (hi2c->xfer_count > MAX_NBYTE_SIZE)
            {
                                    ...
                                    uint32_t timeout_ms)
{
  ...
  hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_ADDR, (uint32_t)RESET, timeout_ms, tick_start);
  if (hal_status == HAL_OK)
  {
    ...
    if (hal_status == HAL_OK)
    {
      ...
      hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_DIR, (uint32_t)RESET, timeout_ms, tick_start);
      if (hal_status == HAL_OK)
      {
        while (hi2c->xfer_count > 0U)
        {
        ...
        if (hal_status == HAL_OK)
        {
          hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_NACKF, (uint32_t)RESET, timeout_ms, tick_start);
          if (hal_status == HAL_OK)
          {
            ...
            if (hal_status == HAL_OK)
            {
              ...
              hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_BUSY, (uint32_t)SET, timeout_ms, tick_start);
            }
          }
        }
      }
...
hal_status_t HAL_I2C_SLAVE_Receive(hal_i2c_handle_t *hi2c, void *p_data, uint32_t size_byte, uint32_t timeout_ms)
{
  ...
  if (hal_status == HAL_OK)
  {
    ...
    hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_DIR, (uint32_t)SET, timeout_ms, tick_start);
    if (hal_status == HAL_OK)
    {
      while (hi2c->xfer_count > 0U)
      {
                                    ...
                                    uint32_t size_byte, uint32_t timeout_ms)
{
  ...
  if (hal_status == HAL_OK)
  {
    ...
    if (hal_status == HAL_OK)
    {
      ...
      {
        /* Wait until RXNE flag is set */
        hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_RXNE, (uint32_t)RESET, timeout_ms, tick_start);
        if (hal_status == HAL_OK)
        {
...
/**
  ...
  * @param  device_addr Target device address: The device 7 bits address value
  *         in datasheet must be shifted to the left before calling the interface
  * @param  trials Number of trials which must greater than or equal to 1
  * @param  timeout_ms Timeout duration in millisecond
  * @retval HAL_OK            Operation completed successfully
  * @retval HAL_BUSY          Concurrent process ongoing or bus is busy
  * @retval HAL_INVALID_PARAM Invalid parameter
  * @retval HAL_TIMEOUT       Operation exceeds user timeout
  */
hal_status_t HAL_I2C_MASTER_IsSlaveReady(hal_i2c_handle_t *hi2c, uint32_t device_addr, uint32_t trials,
                                         uint32_t timeout_ms)
{
  I2C_TypeDef *p_i2cx;
  uint32_t tick_start;
  ...
  do
  {
    ...
    {
      /* Wait until STOPF flag is reset */
      if (I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_STOPF, (uint32_t)RESET, timeout_ms, tick_start) == HAL_OK)
      {
                                           ...
                                           hal_i2c_xfer_opt_t xfer_opt)
{
  ...
  ASSERT_DBG_PARAM((hi2c != NULL) && (p_data != NULL) && (size_byte != 0UL));
  ASSERT_DBG_PARAM(IS_I2C_TRANSFER_OPTIONS_REQUEST(xfer_opt));
  ASSERT_DBG_STATE(hi2c->global_state, I2C_STATE_ALL_LISTEN);
  ...
  if (((uint32_t)hi2c->global_state & I2C_STATE_ALL_LISTEN) == 0U)
  {
    return HAL_BUSY;
  }
  ...
  if ((LL_I2C_GetTransferDirection(p_i2cx) == LL_I2C_DIRECTION_READ) && (tmp != (uint32_t) RESET))
  {
                                          ...
                                          hal_i2c_xfer_opt_t xfer_opt)
{
  ...
  tmp = LL_I2C_IsActiveFlag_ADDR(p_i2cx);
  if ((LL_I2C_GetTransferDirection(p_i2cx) == LL_I2C_DIRECTION_WRITE) && (tmp != (uint32_t) RESET))
  {
    /* Clear ADDR flag after prepare the transfer parameters */
    /* This action generates an acknowledge to the master */
    LL_I2C_ClearFlag_ADDR(p_i2cx);
...
hal_status_t HAL_I2C_MASTER_Abort_IT(hal_i2c_handle_t *hi2c, uint32_t device_addr)
{
  ...
  if (hi2c->mode == HAL_I2C_MODE_MASTER)
  {
    ...
    ASSERT_DBG_STATE(hi2c->global_state, I2C_STATE_ALL_ACTIVE);
...
void HAL_I2C_ER_IRQHandler(hal_i2c_handle_t *hi2c)
{
  ...
  if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_BERR) != RESET)
      && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_ERRIE) != RESET))
  {
  ...
  if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_OVR) != RESET)
      && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_ERRIE) != RESET))
  {
  ...
  if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_ARLO) != RESET)
      && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_ERRIE) != RESET))
  {
...
to asynchronously informed the application in non blocking modes (nterrupt and DMA) :
 ...
 - HAL_I2C_MASTER_MemTxCpltCallback()  : Slave Address Match callback
 - HAL_I2C_MASTER_MemRxCpltCallback()  : Listen Complete callback
 - HAL_I2C_SLAVE_AddrCallback()        : Memory Tx transfer completed callback
 - HAL_I2C_SLAVE_ListenCpltCallback()  : Memory Rx transfer completed callback
 - HAL_I2C_ErrorCallback()             : I2C error callback
 - HAL_I2C_AbortCpltCallback()         : I2C abort callback
 */
...
{
  ASSERT_DBG_PARAM((hi2c != NULL));
  ASSERT_DBG_STATE(hi2c->global_state, ((uint32_t)I2C_STATE_ALL_ACTIVE | (uint32_t)HAL_I2C_STATE_ABORT));
...
static hal_status_t I2C_Master_ISR_IT(hal_i2c_handle_t *hi2c, uint32_t it_flags, uint32_t it_sources)
{
  ...
  if ((I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_NACKF) != RESET)
      && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_NACKIE) != RESET))
  {
    LL_I2C_ClearFlag_NACK(p_i2cx);
  ...
  else if ((I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_RXNE) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_RXIE) != RESET))
  {
  ...
  else if ((I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_TXIS) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_TXIE) != RESET))
  {
    /* Write data to TXDR */
    /* Write data to TXDR */
    LL_I2C_TransmitData8(p_i2cx, *hi2c->p_buf_tx);
  ...
  else if ((I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_TCR) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_TCIE) != RESET))
  {
    if ((hi2c->xfer_count != 0U) && (hi2c->xfer_size == 0U))
    {
    ...
    else
    {
      ...
      if ((LL_I2C_IsEnabledAutoEndMode(p_i2cx) != 1U))
      {
  ...
  else if ((I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_TC) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_TCIE) != RESET))
  {
    if (hi2c->xfer_count == 0U)
    {
  ...
  if ((I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_STOPF) != RESET)
      && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_STOPIE) != RESET))
  {
...
static hal_status_t I2C_Slave_ISR_IT(hal_i2c_handle_t *hi2c, uint32_t it_flags, uint32_t it_sources)
{
  ...
  else if ((I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_ADDR) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_ADDRIE) != RESET))
  {
...
static hal_status_t I2C_Master_ISR_DMA(hal_i2c_handle_t *hi2c, uint32_t it_flags, uint32_t it_sources)
{
  ...
  if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_NACKF) != RESET)
      && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_NACKIE) != RESET))
  {
    LL_I2C_ClearFlag_NACK(p_i2cx);
  ...
  else if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_TCR) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_TCIE) != RESET))
  {
  ...
  else if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_TC) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_TCIE) != RESET))
  {
    if (hi2c->xfer_count == 0U)
    {
  ...
  else if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_STOPF) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_STOPIE) != RESET))
  {
    /* Call I2C master complete process */
...
static hal_status_t I2C_Mem_ISR_DMA(hal_i2c_handle_t *hi2c, uint32_t it_flags, uint32_t it_sources)
{
  ...
  else if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_TXIS) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_TXIE) != RESET))
  {
    /* Write LSB part of Memory Address */
    LL_I2C_TransmitData8(p_i2cx, (uint8_t) hi2c->mem_addr);
...
static hal_status_t I2C_Slave_ISR_DMA(hal_i2c_handle_t *hi2c, uint32_t it_flags, uint32_t it_sources)
{
  ...
  /* Check if STOPF is set */
  if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_STOPF) != RESET)
      && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_STOPIE) != RESET))
  {
    /* Call I2C slave complete process */
      ...
      && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_NACKIE) != RESET))
  {
    ...
    /* Mean xfer_count == 0 */
    /* So clear flag NACKF only */
    if ((I2C_CHECK_IT_SOURCE(it_sources, I2C_CR1_TXDMAEN) != RESET)
        || (I2C_CHECK_IT_SOURCE(it_sources, I2C_CR1_RXDMAEN) != RESET))
    {
      /* Split check of hdma_rx, for MISRA compliance */
      if (hi2c->hdma_rx != NULL)
      {
        if (I2C_CHECK_IT_SOURCE(it_sources, I2C_CR1_RXDMAEN) != RESET)
        {
  ...
  else if ((I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_ADDR) != RESET)
           && (I2C_CHECK_IT_SOURCE(it_sources, LL_I2C_CR1_ADDRIE) != RESET))
  {
                                          ...
                                          uint32_t timeout_ms, uint32_t tick_start)
{
  ...
  if (hal_status == HAL_OK)
  {
    ...
    hal_status = I2C_WaitOnFlagUntilTimeout(hi2c, LL_I2C_ISR_TC, (uint32_t)RESET, timeout_ms, tick_start);
  }
...
static void I2C_ITSlaveSeqCplt(hal_i2c_handle_t *hi2c)
{
  ...
  if (I2C_CHECK_IT_SOURCE(tmp_cr1_value, I2C_CR1_TXDMAEN) != RESET)
  {
    LL_I2C_DisableDMAReq_TX(p_i2cx);
  }
  else if (I2C_CHECK_IT_SOURCE(tmp_cr1_value, I2C_CR1_RXDMAEN) != RESET)
  {
    LL_I2C_DisableDMAReq_RX(p_i2cx);
  }
  else
...
static void I2C_ITMasterCplt(hal_i2c_handle_t *hi2c, uint32_t it_flags)
{
  ...
  if ((hi2c->global_state == HAL_I2C_STATE_ABORT) && (I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_RXNE) != RESET))
  {
    /* Read data from RXDR */
...
static void I2C_ITSlaveCplt(hal_i2c_handle_t *hi2c, uint32_t it_flags)
{
  ...
  /* Store last receive data if any */
  if (I2C_CHECK_FLAG(tmp_it_flags, LL_I2C_ISR_RXNE) != RESET)
  {
    /* Remove RXNE flag on temporary variable as read done */
    tmp_it_flags &= ~LL_I2C_ISR_RXNE;
...
static void I2C_ITListenCplt(hal_i2c_handle_t *hi2c, uint32_t it_flags)
{
  ...
  /* Store last receive data if any */
  if (I2C_CHECK_FLAG(it_flags, LL_I2C_ISR_RXNE) != RESET)
  {
    /* Read data from RXDR */
    *hi2c->p_buf_rx = LL_I2C_ReceiveData8(p_i2cx);
    hi2c->p_buf_rx++;
...
static void I2C_ITError(hal_i2c_handle_t *hi2c, uint32_t error_code)
{
  ...
  /* Disable interrupts */
  if (((uint32_t)tmp_state & I2C_STATE_ALL_LISTEN) != 0U)
  {
                                        ...
                                        uint32_t tick_start)
{
  ...
  if (IS_BIT_SET(tmp_it_flags, LL_I2C_ISR_NACKF))
  {
    LL_I2C_ClearFlag_NACK(p_i2cx);

    /* Wait until STOP flag is set or timeout occurred */
    /* AutoEnd must be initiate after AF */
    while ((LL_I2C_IsActiveFlag_STOP(p_i2cx) == 0U) && (status == HAL_OK))
    {
      ...
      if (timeout_ms != HAL_MAX_DELAY)
      {
        if (((HAL_GetTick() - tick_start_local) > timeout_ms) || (timeout_ms == 0U))
        {
          ...
          if ((LL_I2C_IsActiveFlag_BUSY(p_i2cx) != (uint32_t)RESET)
          ...
          {
            /* Check for the timeout */
            if ((HAL_GetTick() - tick_start_local) > I2C_TIMEOUT_STOPF)
            {
              status = HAL_ERROR;
              break;
            }
  ...
  if (IS_BIT_SET(tmp_it_flags, LL_I2C_ISR_BERR))
  {
#if defined (USE_HAL_I2C_GET_LAST_ERRORS) && (USE_HAL_I2C_GET_LAST_ERRORS == 1)
  ...
  if (IS_BIT_SET(tmp_it_flags, LL_I2C_ISR_OVR))
  {
#if defined (USE_HAL_I2C_GET_LAST_ERRORS) && (USE_HAL_I2C_GET_LAST_ERRORS == 1)
  ...
  if (IS_BIT_SET(tmp_it_flags, LL_I2C_ISR_ARLO))
  {
#if defined (USE_HAL_I2C_GET_LAST_ERRORS) && (USE_HAL_I2C_GET_LAST_ERRORS == 1)
