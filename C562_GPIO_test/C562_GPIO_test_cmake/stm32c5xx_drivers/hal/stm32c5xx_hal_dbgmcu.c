...
2. Debug during low power mode:
   ...
   - Enable, disable the debug module during Sleep, Stop and Standby modes:
   ...
   - Check if the debug module is enabled during Sleep, Stop and Standby modes:
...
#define IS_DBGMCU_GET_DEBUG_LP_MODE(mode)        \
  (((mode) == HAL_DBGMCU_STOP_MODE_DEBUG)        \
   || ((mode) == HAL_DBGMCU_STANDBY_MODE_DEBUG) || ((mode) == HAL_DBGMCU_SLEEP_MODE_DEBUG))
/**
  * @}
  */
