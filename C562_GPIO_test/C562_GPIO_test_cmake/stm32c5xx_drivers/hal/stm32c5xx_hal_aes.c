/**
  ...
  *          This file provides Cryptography AES peripheral services.
  *
  **********************************************************************************************************************
  * @attention
  *
...
This AES HAL driver is a generic driver which contains a set of APIs allowing to configure the AES
...
- Initialize the AES low level resources:
  - Enable the AES peripheral clock:
    ...
    - Or by setting the USE_HAL_AES_CLK_ENABLE_MODEL define to HAL_CLK_ENABLE_PERIPH_ONLY within the
      stm32c5xx_hal_conf.h file, in this case, the AES clock is enabled within
...
- De-initialize the AES peripheral by calling the HAL_AES_DeInit() API that performs these operations:
  - The disable of the AES peripheral
...
- The Configuration of the AES peripheral is ensured through three steps:
  - Step1 (Chaining mode(algorithm) configuration according to the defined compilation algorithm flag):
    ...
    - The AES compilation algorithm flags are:
       - USE_HAL_AES_ECB_CBC_ALGO flag: The AES peripheral can be configured with one of the
...
- Encrypt an amount of data in interrupt mode using HAL_AES_Encrypt_IT()
  - At end of transfer of data between user buffer and AES peripheral, HAL_AES_InCpltCallback() and
...
- Encrypt/decrypt an amount of data after transfer from input user buffer to AES peripheral and get
  encrypted / decrypted data from AES peripheral via DMA using HAL_AES_Encrypt_DMA() or
  HAL_AES_Decrypt_DMA()
  - The minimum data amount transferred with DMA must be equal to one block (four complete words), as the DMA transfer
    ...
    using the DMA)
  - At the end of a transfer of data from the user buffer to the AES peripheral, one of these
  ...
  - At the end of a transfer of data from AES peripheral to the user buffer, one of these callbacks is
   generated:
...
------------------------------ | --------------- | ----------------- | ----------------------------------------
PRODUCT                        | from IDE        | NA                | Ex:STM32C5xx
...
/*! Macro to enable the AES peripheral */
#define AES_ENABLE(handle) (AES_GET_INSTANCE(handle)->CR |=  AES_CR_EN)

/*! Macro to disable the AES peripheral */
...
/** @addtogroup AES_Exported_Functions_Group1
  * @{
This subsection provides a set of functions allowing to initialize and de-initialize the AES
peripheral:
...
/** @addtogroup AES_Exported_Functions_Group2
  * @{
This subsection provides a set of functions allowing to configure the AES peripheral through
three steps:
- Step1: Chaining mode(algorithm) configuration APIs:
       - HAL_AES_ECB_SetConfig()      :Allowing to configure the AES peripheral with the ECB algorithm
       - HAL_AES_CBC_SetConfig()      :Allowing to configure the AES peripheral with the CBC algorithm
...
#if defined(USE_HAL_AES_ECB_CBC_ALGO) && (USE_HAL_AES_ECB_CBC_ALGO == 1)
/**
  * @brief  Configure the AES peripheral with ECB algorithm.
  * @param  haes      Pointer to a @ref hal_aes_handle_t structure
  * @note   The user has not to set any parameter
  * @retval HAL_INVALID_PARAM Invalid param return when the AES handle is NULL
  * @retval HAL_OK    AES peripheral has been correctly configured with ECB algorithm
  */
hal_status_t HAL_AES_ECB_SetConfig(hal_aes_handle_t *haes)
{
  ...
  AES_DISABLE(haes);
  MODIFY_REG(AES_GET_INSTANCE(haes)->CR, AES_CR_CHMOD | AES_CR_DATATYPE, AES_ALGORITHM_ECB);
...
/**
  * @brief  Configure the AES peripheral with CBC algorithm.
  * @param  haes              Pointer to a @ref hal_aes_handle_t structure
  ...
  * @retval HAL_OK            AES peripheral has been correctly configured with CBC algorithm
  */
hal_status_t HAL_AES_CBC_SetConfig(hal_aes_handle_t *haes, const uint32_t *p_init_vect)
{
  ...
  AES_DISABLE(haes);
  MODIFY_REG(AES_GET_INSTANCE(haes)->CR, AES_CR_CHMOD | AES_CR_DATATYPE, AES_ALGORITHM_CBC);
...
/**
  * @brief  Configure the AES peripheral with the CTR algorithm.
  * @param  haes              Pointer to a @ref hal_aes_handle_t structure
...
#if defined(USE_HAL_AES_GCM_GMAC_ALGO) && (USE_HAL_AES_GCM_GMAC_ALGO == 1)
/**
  * @brief  Configure the AES peripheral with the GCM_GMAC algorithm.
  * @param  haes              Pointer to a @ref hal_aes_handle_t structure
...
/**
  * @brief  Configure the AES with the CCM algorithm.
  * @param  haes              Pointer to a @ref hal_aes_handle_t structure
...
/**
  * @brief  Configure the AES Normal key(key size, key value).
  * @param  haes              Pointer to a @ref hal_aes_handle_t structure
                             ...
                             uint32_t timeout_ms)
{
  ...
  MODIFY_REG(AES_GET_INSTANCE(haes)->CR, AES_CR_MODE, AES_OPERATING_MODE_ENCRYPT);
                             ...
                             uint32_t timeout_ms)
{
  ...
  MODIFY_REG(AES_GET_INSTANCE(haes)->CR, AES_CR_MODE, AES_OPERATING_MODE_DECRYPT);
...
hal_status_t HAL_AES_SaveContext(hal_aes_handle_t *haes, hal_aes_save_context_t *p_context)
{
  ...
  if ((haes->algorithm == AES_ALGORITHM_GCM_GMAC) || (haes->algorithm == AES_ALGORITHM_CCM))
  {
    p_context->SUSPRx[0] = AES_GET_INSTANCE(haes)->SUSPR7;
    p_context->SUSPRx[1] = AES_GET_INSTANCE(haes)->SUSPR6;
    p_context->SUSPRx[2] = AES_GET_INSTANCE(haes)->SUSPR5;
    p_context->SUSPRx[3] = AES_GET_INSTANCE(haes)->SUSPR4;
    p_context->SUSPRx[4] = AES_GET_INSTANCE(haes)->SUSPR3;
    p_context->SUSPRx[5] = AES_GET_INSTANCE(haes)->SUSPR2;
    p_context->SUSPRx[6] = AES_GET_INSTANCE(haes)->SUSPR1;
    p_context->SUSPRx[7] = AES_GET_INSTANCE(haes)->SUSPR0;
  }
#endif /* USE_HAL_AES_GCM_GMAC_ALGO or USE_HAL_AES_CCM_ALGO */
...
hal_status_t HAL_AES_RestoreContext(hal_aes_handle_t *haes, const hal_aes_save_context_t *p_context)
{
  ...
  if ((haes->algorithm == AES_ALGORITHM_GCM_GMAC) || (haes->algorithm == AES_ALGORITHM_CCM))
  {
    AES_GET_INSTANCE(haes)->SUSPR7 = p_context->SUSPRx[0];
    AES_GET_INSTANCE(haes)->SUSPR6 = p_context->SUSPRx[1];
    AES_GET_INSTANCE(haes)->SUSPR5 = p_context->SUSPRx[2];
    AES_GET_INSTANCE(haes)->SUSPR4 = p_context->SUSPRx[3];
    AES_GET_INSTANCE(haes)->SUSPR3 = p_context->SUSPRx[4];
    AES_GET_INSTANCE(haes)->SUSPR2 = p_context->SUSPRx[5];
    AES_GET_INSTANCE(haes)->SUSPR1 = p_context->SUSPRx[6];
    AES_GET_INSTANCE(haes)->SUSPR0 = p_context->SUSPRx[7];
  }
#endif /* USE_HAL_AES_GCM_GMAC_ALGO or USE_HAL_AES_CCM_ALGO */
...
user input/output buffer and the AES peripheral:
...
/**
  ...
  * @param  haes                    Pointer to a @ref hal_aes_handle_t structure
  * @retval HAL_AES_STATE_RESET     AES peripheral is not yet initialized
  * @retval HAL_AES_STATE_INIT      AES peripheral is initialized but not yet configured
  * @retval HAL_AES_STATE_IDLE      AES peripheral is initialized and configured
...
#if defined(USE_HAL_AES_CCM_ALGO) && (USE_HAL_AES_CCM_ALGO == 1)
/**
  * @brief  Generate the CCM authentication TAG.
  *         This API can only be called after accomplishing either an encryption or a decryption process
  * @param  haes              Pointer to a @ref hal_aes_handle_t structure
  * @param  p_auth_tag        Pointer to the authentication buffer the p_auth_tag generated here is 128bits length,
  *                           if the TAG length is less than 128bits, user must consider only the valid part of
...
This subsection provides a set of functions allowing to:
 ...
 - Manage data transfer between user buffers and the AES peripheral:
...
static hal_status_t AES_WaitForSetKey(hal_aes_handle_t *haes)
{
  ...
  while (HAL_AES_GetFlag(haes, HAL_AES_FLAG_KEYVALID) != 1U)
  {
