...
1. Declare a hal_smartcard_handle_t handle structure
  - For example: hal_smartcard_handle_t hsmartcard;

2. Configure the low level hardware (GPIO, CLOCK, NVIC...etc):
  - Enable the SMARTCARDx clock if USE_HAL_SMARTCARD_CLK_ENABLE_MODEL = HAL_CLK_ENABLE_NO
...
3. Initialize the SMARTCARDx driver with an USART HW instance
  - Call HAL_SMARTCARD_Init() function with the selected instance
  - The SMARTCARDx clock is enabled inside the HAL_SMARTCARD_Init() if
    USE_HAL_SMARTCARD_CLK_ENABLE_MODEL > HAL_CLK_ENABLE_NO.

4. Configure the communication baud rate, stop bit, first bit, parity mode, NACK, smartcard clock prescaler,
   source clock prescaler, clock polarity, clock phase, clock output enabling, guard time and auto retry count
   by calling HAL_SMARTCARD_SetConfig().
   @note In SMARTCARD case, etu (Elementary Time Unit) is equivalent to the baud period duration

5. Configure and/or enable advanced features.
  - HAL_SMARTCARD_EnableIOInversion() to invert the IO pin active level logic
...
6. For SMARTCARDx IO operations modes, polling, interrupt and DMA are available
   within this driver.
  ...
  - DMA mode IO operation
    ...
    - At transmission half of transfer HAL_SMARTCARD_TxHalfCpltCallback() is executed and user can
      add his own code by customization of function pointer HAL_SMARTCARD_TxHalfCpltCallback()
    - At transmission end of transfer HAL_SMARTCARD_TxCpltCallback() is executed and user can
      add his own code by customization of function pointer HAL_SMARTCARD_TxCpltCallback()
    - Receive an amount of data in non-blocking mode (DMA) using HAL_SMARTCARD_Receive_DMA()
    - At reception half of transfer HAL_SMARTCARD_RxHalfCpltCallback() is executed and user can
      add his own code by customization of function pointer HAL_SMARTCARD_RxHalfCpltCallback()
    - At reception end of transfer HAL_SMARTCARD_RxCpltCallback() is executed and user can
      add his own code by customization of function pointer HAL_SMARTCARD_RxCpltCallback()
    - In case of transfer Error, HAL_SMARTCARD_ErrorCallback() function is executed and user can
      add his own code by customization of function pointer HAL_SMARTCARD_ErrorCallback()
  ...
  - Different sequential SMARTCARD interfaces are listed below:
    ...
    - End of abort IT process, HAL_SMARTCARD_AbortCpltCallback() is executed and users can add their own code
      by customization of function pointer HAL_SMARTCARD_AbortCpltCallback()
...
7. Callback registration
  - When the compilation flag USE_HAL_SMARTCARD_REGISTER_CALLBACKS is set to 1, it allows the user to configure
    dynamically the driver callbacks via its own method:
...
8. Acquire/Release the SMARTCARD bus
  - When the compilation flag USE_HAL_MUTEX is set to 1, it allows the user to acquire/reserve the whole SMARTCARD
    Interface for executing process.
  ...
  - When the compilation flag USE_HAL_MUTEX is set to 0 or not defined,
    HAL_SMARTCARD_AcquireBus() and HAL_SMARTCARD_ReleaseBus() are not available.

9. Configuration inside the SMARTCARD driver

Config defines                       | Description      | Default value | Note
-------------------------------------| ---------------- | ------------- | ---------------------------------------------
USE_HAL_SMARTCARD_CLK_ENABLE_MODEL   | from hal_conf.h  | HAL_CLK_ENABLE_NO | Enable the gating of the peripheral clock
USE_HAL_SMARTCARD_MODULE             | from hal_conf.h  | 1  | When set, HAL SMARTCARD module is enabled
USE_HAL_SMARTCARD_GET_LAST_ERRORS    | from hal_conf.h  | 0  | Enable the get last process error
USE_HAL_SMARTCARD_USER_DATA          | from hal_conf.h  | 0  | Enable the set/get user data
USE_HAL_SMARTCARD_REGISTER_CALLBACKS | from hal_conf.h  | 0  | Enable the register callbacks assert
USE_HAL_CHECK_PARAM                  | from hal_conf.h  | 0  | Parameters (pointers or sizes) are checked in runtime
USE_HAL_CHECK_PROCESS_STATE          | from hal_conf.h  | 0  | Enable atomic access to process state check
USE_HAL_MUTEX                        | from hal_conf.h  | 0  | Enable the acquire/release bus services
USE_HAL_SMARTCARD_DMA                | from hal_conf.h  | 1  | Enable the DMA feature
USE_HAL_SMARTCARD_FIFO               | from hal_conf.h  | 1  | Enable the FIFO feature
...
#ifdef USE_HAL_SMARTCARD_MODULE
/* Private define ------------------------------------------------------------*/
...
/** @brief  Check SMARTCARD Baud rate.
  * @param  baud_rate Baudrate specified by the user.
  *         The maximum Baud Rate is derived from the maximum clock on C5 (i.e. 144 MHz)
  *         divided by the oversampling method used on the USART (i.e. 16)
...
/** @brief  Check SMARTCARD Receiver Timeout value.
  ...
  * @retval SET (timeout_etu is valid) or RESET (timeout_etu is invalid)
  */
...
/**
  ...
  * @retval SET (first_bit is valid) or RESET (first_bit is invalid)
  */
...
/**
  ...
  * @retval SET (nack is valid) or RESET (nack is invalid)
  */
...
/**
  ...
  * @retval SET (clockoutput is valid) or RESET (clockoutput is invalid)
  */
...
/**
  ...
  * @retval SET (clock_phase is valid) or RESET (clock_phase is invalid)
  */
...
/**
  ...
  * @retval SET (retry count is valid) or RESET (retry count is invalid)
  */
...
/**
  ...
  * @retval SET (guard time is valid) or RESET (guard time is invalid)
  */
...
/**
  ...
  * @retval SET (indication is valid) or RESET (indication is invalid)
  */
...
/**
  ...
  * @retval SET (clock_prescaler is valid) or RESET (clock_prescaler is invalid)
  */
...
/**
  ...
  * @retval SET (block_length_byte is valid) or RESET (block_length_byte is invalid)
  */
...
/**
  ...
  * @retval SET (sclk_prescaler is valid) or RESET (sclk_prescaler is invalid)
  */
...
void SMARTCARD_InitCallbacksToDefault(hal_smartcard_handle_t *hsmartcard);
#endif /* USE_HAL_SMARTCARD_REGISTER_CALLBACKS */
static hal_status_t SMARTCARD_WaitOnFlagUntilTimeout(hal_smartcard_handle_t *hsmartcard, uint32_t flag,
                                                     FlagStatus Status, uint32_t tickstart, uint32_t timeout_ms);
...
#endif /* USE_HAL_SMARTCARD_FIFO */
#if defined(USE_HAL_SMARTCARD_CLK_ENABLE_MODEL) && (USE_HAL_SMARTCARD_CLK_ENABLE_MODEL == HAL_CLK_ENABLE_PERIPH_ONLY)
...
hal_status_t HAL_SMARTCARD_DeInit(hal_smartcard_handle_t *hsmartcard)
{
  ...
  if ((temp_state == HAL_SMARTCARD_STATE_RX_ACTIVE) || (temp_state == HAL_SMARTCARD_STATE_TX_ACTIVE))
  {
    ...
    (void)HAL_SMARTCARD_Abort(hsmartcard);
  }
...
/** @addtogroup SMARTCARD_Exported_Functions_Group2 General Config functions
  ...
  */
/**
  * @brief  Configure the SMARTCARD according to the user parameters into the handler instance registers.
...
hal_status_t HAL_SMARTCARD_SetConfig(hal_smartcard_handle_t *hsmartcard, const hal_smartcard_config_t *p_config)
{
  ...
  reg_temp = (reg_temp & ~(USART_GTPR_GT)) | (p_config->guard_time_etu << USART_GTPR_GT_Pos);
  reg_temp = (reg_temp & ~(USART_GTPR_PSC)) | ((uint32_t)p_config->sclk_prescaler);
  LL_USART_WRITE_REG(p_smartcardx, GTPR, reg_temp);
  reg_temp = LL_USART_READ_REG(p_smartcardx, CR3);
  reg_temp = (reg_temp & ~(USART_CR3_SCARCNT)) | (p_config->auto_retry_count << USART_CR3_SCARCNT_Pos);

  /* Enabling Smarcard Mode */
  reg_temp = (reg_temp | (USART_CR3_SCEN));
  reg_temp = (reg_temp & ~(USART_CR3_NACK)) | ((uint32_t)p_config->nack << USART_CR3_NACK_Pos);
  LL_USART_WRITE_REG(p_smartcardx, CR3, reg_temp);
  reg_temp = LL_USART_READ_REG(p_smartcardx, CR2);
  reg_temp = (reg_temp & ~(USART_CR2_MSBFIRST)) | ((uint32_t)p_config->first_bit);
  reg_temp = (reg_temp & ~(USART_CR2_STOP)) | ((uint32_t)p_config->stop_bits);

  /* Last Bit clock Pulse enabled */
  reg_temp = (reg_temp | (USART_CR2_LBCL));
  reg_temp = (reg_temp & ~(USART_CR2_CLKEN)) | ((uint32_t)p_config->clock_output);
  reg_temp = (reg_temp & ~(USART_CR2_CPHA)) | ((uint32_t)p_config->clock_phase);
  reg_temp = (reg_temp & ~(USART_CR2_CPOL)) | ((uint32_t)p_config->clock_polarity);
  LL_USART_WRITE_REG(p_smartcardx, CR2, reg_temp);
  reg_temp = LL_USART_READ_REG(p_smartcardx, CR1);
  reg_temp = (reg_temp & ~(USART_CR1_PS)) | ((uint32_t)p_config->parity);

  /* Configure transfer as 8 bit + parity */
  reg_temp = (reg_temp & ~(USART_CR1_M1)) | (LL_USART_DATAWIDTH_9_BIT);
  reg_temp = (reg_temp & ~(USART_CR1_OVER8)) | (LL_USART_OVERSAMPLING_16);

  /* Enabling Parity control */
  reg_temp = (reg_temp | (USART_CR1_PCE));
  LL_USART_WRITE_REG(p_smartcardx, CR1, reg_temp);
  ...
  LL_USART_SetBaudRate(p_smartcardx, instance_clock_freq, p_config->clock_prescaler, LL_USART_OVERSAMPLING_16,
                       p_config->baud_rate);
...
hal_status_t HAL_SMARTCARD_GetConfig(const hal_smartcard_handle_t *hsmartcard, hal_smartcard_config_t *p_config)
{
  ...
  p_config->baud_rate = LL_USART_GetBaudRate(p_smartcardx, instance_clock_freq, p_config->clock_prescaler,
...
/** @addtogroup SMARTCARD_Exported_Functions_Group3 Unitary basic config functions
  * @{
  ...
  - Unitary configuration :
    ...
    |---------------------|:-----------------------------------:|
    | Baudrate            |                 0U                  | (This value must be changed)
    | StopBits            |      HAL_SMARTCARD_STOP_BIT_1       | (does not exist in smartcard context)
...
/**
  ...
  */
hal_status_t HAL_SMARTCARD_SetGuardTime(const hal_smartcard_handle_t *hsmartcard, const uint32_t guard_time_etu)
{
  USART_TypeDef *p_smartcardx;
  ...
  * @{
  ...
  - Rx overrun :
    ...
    - HAL_SMARTCARD_DisableRxOverRunDetection(), disable Rx overrun errors detection into the handler
      instance registers.
    - HAL_SMARTCARD_IsEnabledRxOverRunDetection(), fetch Rx overrun errors detection status from the handler
      instance registers.
...
hal_status_t HAL_SMARTCARD_SetTxCpltIndication(hal_smartcard_handle_t *hsmartcard,
                                               const hal_smarcard_tx_cplt_guard_time_indication_t tx_cplt_indication)
{
  ASSERT_DBG_PARAM(hsmartcard != NULL);
...
/**
  ...
  * @param  hsmartcard Pointer to a \ref hal_smartcard_handle_t
  * @retval hal_smarcard_tx_cplt_guard_time_indication_t  SMARTCARD Pre guard time Tx complete indication
  */
...
/** @addtogroup SMARTCARD_Exported_Functions_Group5 FIFO config functions
  * @{
  ...
  Prior to use the fifo mode feature, one has to configure one's instance with HAL_SMARTCARD_SetConfig().
...
hal_status_t HAL_SMARTCARD_EnableFifoMode(hal_smartcard_handle_t *hsmartcard)
{
  ...
  ASSERT_DBG_PARAM(hsmartcard != NULL);
  ASSERT_DBG_PARAM(IS_UART_FIFO_INSTANCE((USART_TypeDef *)(uint32_t)hsmartcard->instance));
...
/**
  ...
  * @param  hsmartcard Pointer to a \ref hal_smartcard_handle_t
  * @param  threshold SMARTCARD Tx FIFO threshold value
                                    ...
                                    uint16_t size_byte, uint32_t timeout_ms)
{
  ...
  if (LL_USART_IsEnabledDMAReq_TX(p_smartcardx) == 1U)
  {
  ...
  while (hsmartcard->tx_xfer_count > 0U)
  {
    ...
    if (SMARTCARD_WaitOnFlagUntilTimeout(hsmartcard, LL_USART_ISR_TXE_TXFNF, RESET, tickstart, timeout_ms) != HAL_OK)
    {
      return HAL_TIMEOUT;
    }
                                   ...
                                   uint16_t size_byte, uint32_t timeout_ms)
{
...
#if defined(USART_DMAREQUESTS_SW_WA)
  if (LL_USART_IsEnabledDMAReq_RX(p_smartcardx) == 1U)
  {
  ...
  while (hsmartcard->rx_xfer_count > 0U)
  {
    ...
    if (SMARTCARD_WaitOnFlagUntilTimeout(hsmartcard, LL_USART_ISR_RXNE_RXFNE, RESET, tickstart, timeout_ms) != HAL_OK)
    {
      return HAL_TIMEOUT;
    }
...
hal_status_t HAL_SMARTCARD_Transmit_DMA(hal_smartcard_handle_t *hsmartcard, const uint8_t *p_data, uint16_t size_byte)
{
  ...
  return SMARTCARD_Start_Transmit_DMA(hsmartcard, (const uint8_t *)p_data, size_byte, HAL_SMARTCARD_OPT_DMA_TX_IT_HT);
}
                                            ...
                                            uint16_t size_byte, uint32_t interrupts)
{
  ...
  return SMARTCARD_Start_Transmit_DMA(hsmartcard, (const uint8_t *)p_data, size_byte, interrupts);
                                         ...
                                         uint32_t interrupts)
{
  ...
  hsmartcard->p_tx_buff         = (uint8_t *)p_data;
...
static void SMARTCARD_FLUSH_DRREGISTER(hal_smartcard_handle_t *hsmartcard)
{
  ...
  do
  {
    LL_USART_SetRequest(p_smartcardx, (uint16_t) LL_USART_REQUEST_RX_DATA_FLUSH);
    LL_USART_SetRequest(p_smartcardx, (uint16_t) LL_USART_REQUEST_TX_DATA_FLUSH);
  } while (0U);
}
