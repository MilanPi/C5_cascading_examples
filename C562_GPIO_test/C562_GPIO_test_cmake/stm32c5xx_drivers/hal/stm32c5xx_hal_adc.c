...
#define IS_ADC_REG_TRIGGER_SRC_ADC12(trigger_src)  (((trigger_src) == HAL_ADC_REG_TRIG_SOFTWARE_START) \
                                                    ...
                                                    || ((trigger_src) == HAL_ADC_REG_TRIG_TIM5_CH4) \
...
#define IS_ADC_INJ_TRIGGER_SRC_ADC12(trigger_src)  (((trigger_src) == HAL_ADC_INJ_TRIG_SOFTWARE_START) \
                                                    ...
                                                    || ((trigger_src) == HAL_ADC_INJ_TRIG_TIM5_CH1) \
                                                    || ((trigger_src) == HAL_ADC_INJ_TRIG_TIM5_CH2) \
                                                    || ((trigger_src) == HAL_ADC_INJ_TRIG_TIM5_CH3) \
                                                    ...
                                                    || ((trigger_src) == HAL_ADC_INJ_TRIG_LPTIM1_CH1) \
...
/*! Assert definitions of ADC channel parameters */
...
#define IS_ADC_SAMPLING_TIME(sampling_time) (((sampling_time) ==  HAL_ADC_SAMPLING_TIME_2CYCLES_5) \
                                             || ((sampling_time) ==  HAL_ADC_SAMPLING_TIME_4CYCLES_5) \
                                             || ((sampling_time) ==  HAL_ADC_SAMPLING_TIME_7CYCLES_5) \
                                             || ((sampling_time) ==  HAL_ADC_SAMPLING_TIME_12CYCLES_5) \
                                             || ((sampling_time) ==  HAL_ADC_SAMPLING_TIME_24CYCLES_5) \
                                             || ((sampling_time) ==  HAL_ADC_SAMPLING_TIME_47CYCLES_5) \
                                             || ((sampling_time) ==  HAL_ADC_SAMPLING_TIME_138CYCLES_5) \
                                             || ((sampling_time) ==  HAL_ADC_SAMPLING_TIME_288CYCLES_5))
...
/**
  ...
  *         Refer to device reference manual of macro "ADC_COMMON_INSTANCE()" returning
                                      ...
                                      const hal_adc_channel_config_t *p_config)
{
  ...
  ASSERT_DBG_STATE(hadc->global_state,
                   (uint32_t)HAL_ADC_STATE_CONFIGURING |
                   (uint32_t)HAL_ADC_STATE_IDLE);
  if (hadc->global_state == HAL_ADC_STATE_CONFIGURING)
  {
...
hal_status_t HAL_ADC_MM_SetConfig(hal_adc_handle_t *hadc, const hal_adc_mm_config_t *p_config)
{
      ...
      || (p_config->mode == HAL_ADC_MM_DUAL_REG_INT_INJ_SIM))
  {
    ...
    {
      ASSERT_DBG_PARAM(p_config->interl_delay <= HAL_ADC_MM_INTERL_DELAY_8CYCLES);
    }
    else if (adc_resolution == LL_ADC_RESOLUTION_6B)
    {
      ASSERT_DBG_PARAM(p_config->interl_delay <= HAL_ADC_MM_INTERL_DELAY_6CYCLES);
    }
    else
    {
      /* No action */
...
hal_status_t HAL_ADC_GetCalibrationFactor(hal_adc_handle_t *hadc, hal_adc_calib_t *p_calib)
{
  ...
  p_calib->factors[0] = LL_ADC_GetCalibrationFactor(p_instance, HAL_ADC_IN_SINGLE_ENDED);
...
hal_status_t HAL_ADC_SetCalibrationFactor(hal_adc_handle_t *hadc, const hal_adc_calib_t *p_calib)
{
  ...
  LL_ADC_SetCalibrationFactor(p_instance, HAL_ADC_IN_SINGLE_ENDED, p_calib->factors[0]);
...
/**
  ...
  * @note   This function can be used to iterate a conversion process on going initiated
...
hal_status_t adc_calibrate(hal_adc_handle_t *hadc)
{
  ...
  if (status == HAL_OK)
  {
    ...
    LL_ADC_StartCalibration(p_instance, HAL_ADC_IN_SINGLE_ENDED);
