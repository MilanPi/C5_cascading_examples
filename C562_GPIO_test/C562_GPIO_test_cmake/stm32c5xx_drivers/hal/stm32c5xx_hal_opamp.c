...
#define IS_OPAMP_INVERTING_INPUT(input) (((input) ==  HAL_OPAMP_INVERTING_INPUT_GPIO_0)  \
                                         ...
                                         || ((input) ==  HAL_OPAMP_INVERTING_INPUT_OPAMP_OUT ))
...
#define IS_OPAMP_MUX_INPUT_CTRL(mux_inputs_ctrl) (((mux_inputs_ctrl) == HAL_OPAMP_MUX_INPUTS_CTRL_DISABLE) \
                                                  || ((mux_inputs_ctrl) == HAL_OPAMP_MUX_INPUTS_CTRL_TIM1_CH6) \
                                                  || ((mux_inputs_ctrl) == HAL_OPAMP_MUX_INPUTS_CTRL_TIM2_CH4) \
                                                  || ((mux_inputs_ctrl) == HAL_OPAMP_MUX_INPUTS_CTRL_TIM8_CH6) \
                                                  || ((mux_inputs_ctrl) == HAL_OPAMP_MUX_INPUTS_CTRL_TIM12_CH1) \
                                                  || ((mux_inputs_ctrl) == HAL_OPAMP_MUX_INPUTS_CTRL_TIM15_CH2) \
                                                  || ((mux_inputs_ctrl) == HAL_OPAMP_MUX_INPUTS_CTRL_TIM20_CH6))
...
#define IS_OPAMP_MUX_PGA_GAIN_CTRL(mux_pga_ctrl) (((mux_pga_ctrl) == HAL_OPAMP_MUX_PGA_GAIN_CTRL_DISABLE) \
                                                  || ((mux_pga_ctrl) == HAL_OPAMP_MUX_PGA_GAIN_CTRL_TIM1_CH6) \
                                                  || ((mux_pga_ctrl) == HAL_OPAMP_MUX_PGA_GAIN_CTRL_TIM3_CH2) \
                                                  || ((mux_pga_ctrl) == HAL_OPAMP_MUX_PGA_GAIN_CTRL_TIM8_CH6) \
                                                  || ((mux_pga_ctrl) == HAL_OPAMP_MUX_PGA_GAIN_CTRL_TIM9_CH1) \
                                                  || ((mux_pga_ctrl) == HAL_OPAMP_MUX_PGA_GAIN_CTRL_TIM15_CH2) \
                                                  || ((mux_pga_ctrl) == HAL_OPAMP_MUX_PGA_GAIN_CTRL_TIM20_CH6))
...
hal_status_t HAL_OPAMP_SetConfig(hal_opamp_handle_t *hopamp, const hal_opamp_config_t *p_config)
{
  ...
  switch (p_config->configuration_mode)
  {
    case HAL_OPAMP_MODE_PGA:
      ...
      non_inverting_input     = HAL_OPAMP_NON_INVERTING_INPUT_GPIO_0;
      break;

    case HAL_OPAMP_MODE_FOLLOWER:
      inverting_input         = HAL_OPAMP_INVERTING_INPUT_OPAMP_OUT;
      non_inverting_input     = HAL_OPAMP_NON_INVERTING_INPUT_GPIO_0;
      break;
...
void HAL_OPAMP_GetConfig(const hal_opamp_handle_t *hopamp, hal_opamp_config_t *p_config)
{
  ...
  p_config->configuration_mode = (hal_opamp_config_mode_t)(((reg_value & OPAMP_CSR_VM_SEL_1) == 0UL)
                                                           ? (LL_OPAMP_MODE_STANDALONE) : (reg_value & OPAMP_CSR_VM_SEL)
                                                          );
}
                                 ...
                                 hal_opamp_power_mode_t power_mode)
{
  ...
  p_config->trim_offset_n  = (read_value & OPAMP_CSR_TRIMOFFSETN_Msk) >> OPAMP_CSR_TRIMOFFSETN_Pos;
  p_config->trim_offset_p  = (read_value & OPAMP_CSR_TRIMOFFSETP_Msk) >> OPAMP_CSR_TRIMOFFSETP_Pos;
}
                                                            ...
                                                            const hal_opamp_config_input_connection_t *p_config)
{
  ...
  /* Set OPAMP inputs secondary connections */
  LL_OPAMP_SetInputsMuxSecondary(p_instance, (uint32_t)p_config->non_inverting_input,
                                 (uint32_t)p_config->inverting_input);

  return HAL_OK;
}

/**
  * @brief  Set the PGA secondary gain to be used when the OPAMP is configured in Programmable Gain Amplifier.
  * @param  hopamp Pointer to a hal_opamp_handle_t structure.
...
hal_status_t HAL_OPAMP_SetMuxInputsCtrl(const hal_opamp_handle_t *hopamp, hal_opamp_input_crtl_tim_t mux_inputs_ctrl)
{
  ...
  p_instance = OPAMP_GET_INSTANCE(hopamp);
  LL_OPAMP_SetMuxInputsCtrl(p_instance, (uint32_t)mux_inputs_ctrl);
...
/**
  ...
  *         the specified parameters in the hal_opamp_input_crtl_tim_t.
  * @param  hopamp Pointer to a hal_opamp_handle_t structure.
  * @retval value of type hal_opamp_input_crtl_tim_t
  */
hal_opamp_input_crtl_tim_t HAL_OPAMP_GetMuxInputsCtrl(const hal_opamp_handle_t *hopamp)
{
  OPAMP_TypeDef *p_instance;
  hal_opamp_input_crtl_tim_t mux_inputs_ctrl;
  ...
  mux_inputs_ctrl = (hal_opamp_input_crtl_tim_t) LL_OPAMP_GetMuxInputsCtrl(p_instance);
...
/**
  * @brief  Get the Timer signal selection for OPAMP input control
  *         the specified parameters in the hal_opamp_pga_crtl_tim_t.
  * @param  hopamp Pointer to a hal_opamp_handle_t structure.
  * @retval value of type hal_opamp_pga_crtl_tim_t
  */
hal_opamp_pga_crtl_tim_t HAL_OPAMP_GetMuxPGAGainCtrl(const hal_opamp_handle_t *hopamp)
{
  OPAMP_TypeDef *p_instance;
  hal_opamp_pga_crtl_tim_t mux_pga_ctrl;
  ...
  mux_pga_ctrl = (hal_opamp_pga_crtl_tim_t) LL_OPAMP_GetMuxPGAGainCtrl(p_instance);
                                    ...
                                    hal_opamp_power_mode_t power_mode, uint32_t max_nb_opamp)
{
  ...
  uint32_t memo_opamp_mode[OPAMP_INSTANCES_NB];
  uint32_t memo_opamp_output[OPAMP_INSTANCES_NB];
  uint32_t trim_value[OPAMP_INSTANCES_NB];
  ...
  for (index = 0U; index < nb_instance; index++)
  {
    ...
    memo_opamp_output[index] = LL_OPAMP_GetOutputConnection(opamp_instance[index]);
    ...
    LL_OPAMP_SetOutputConnection(opamp_instance[index], LL_OPAMP_OUTPUT_CONNECT_EXTERNAL);
  ...
  for (uint32_t loop = 0U ; loop < 2U; loop++)
  {
    ...
    /* Indeed the first value that causes the CALOUT bit to change from 1 to 0  */
  ...
  for (index = 0U; index < nb_instance; index++)
  {
    ...
    LL_OPAMP_SetOutputConnection(opamp_instance[index], memo_opamp_output[index]);
  }
}
#else
/**
