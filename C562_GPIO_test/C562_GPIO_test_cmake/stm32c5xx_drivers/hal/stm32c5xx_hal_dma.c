...
- As optional, set the DMA channel direct transfer feature configuration :
  ...
  - Use HAL_DMA_SetConfigDirectXferHardwareRequestMode() function to set the transfer hardware request mode
    configuration
  - Use HAL_DMA_SetConfigDirectXferFlowControlMode() function to set the transfer flow control mode configuration
...
- At node level, the operations that can be done are filling a new linked-list node or get a linked-list node
  ...
  - As optional, fill the DMA channel linked-list node feature configuration :
    ...
    - Use HAL_DMA_FillNodeFlowControlMode() function to fill the DMA linked-list node flow control mode configuration
...
#define IS_DMA_REQUEST(value)                                                 \
  (((value) == HAL_DMA_REQUEST_SW) || ((value) <= HAL_LPDMA2_REQUEST_XSPI1_RX))
...
hal_status_t HAL_DMA_Init(hal_dma_handle_t *hdma, hal_dma_channel_t instance)
{
  ...
  {
    /*HAL_RCC_LPDMA1_EnableClock(); To be uncomment when HAL available*/
    LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_LPDMA1);
  }
  else
  {
    /*HAL_RCC_LPDMA2_EnableClock(); To be uncomment when HAL available*/
...
hal_status_t HAL_DMA_FillNodeFlowControlMode(hal_dma_node_t *p_node, hal_dma_flow_control_mode_t flow_control_mode)
{
  ...
  MODIFY_REG(p_node->regs[DMA_NODE_CTR2_REG_OFFSET], DMA_CTR2_PFREQ, (uint32_t)flow_control_mode);
