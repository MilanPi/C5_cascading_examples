...
/** @defgroup  AES_Error_Code Error Code definition reflecting the processes asynchronous errors
  * @{
  */
...
#define  HAL_AES_ERROR_READ   AES_SR_RDERR      /*!< Read error  */
#define  HAL_AES_ERROR_WRITE  AES_SR_WRERR      /*!< Write error */
...
#define HAL_AES_FLAG_BUSY     AES_SR_BUSY                 /*!< Process suspension forbidden
                                                               */
#define HAL_AES_FLAG_WRERR    (AES_SR_WRERR | 0x80000000U) /*!< Write Error flag */
#define HAL_AES_FLAG_RDERR    (AES_SR_RDERR | 0x80000000U) /*!< Read error  flag */
...
typedef enum
{
  HAL_AES  = (uint32_t)AES, /*!< AES  instance */
} hal_aes_t;
...
__STATIC_INLINE uint32_t HAL_AES_GetFlag(const hal_aes_handle_t *haes, uint32_t flag)
{
  ...
  {
    status = ((READ_BIT(((AES_TypeDef *)((uint32_t)haes->instance))->SR,
  ...
  {
    status = ((READ_BIT(((AES_TypeDef *)((uint32_t)haes->instance))->ISR, flag) == flag) ? 1U : 0U);
  }
  else
  {
    status = 0;
...
{
  SET_BIT(((AES_TypeDef *)((uint32_t)haes->instance))->ICR, AES_ICR_RWEIF);
}
...
{
  SET_BIT(((AES_TypeDef *)((uint32_t)haes->instance))->ICR, AES_ICR_CCF);
}
...
{
  SET_BIT(((AES_TypeDef *)((uint32_t)haes->instance))->ICR, AES_ICR_KEIF);
}
...
{
  return ((READ_BIT(((AES_TypeDef *)((uint32_t)haes->instance))->IER, (uint32_t)interrupt)
...
{
  SET_BIT(((AES_TypeDef *)((uint32_t)haes->instance))->IER, interrupt);
}
...
{
  CLEAR_BIT(((AES_TypeDef *)((uint32_t)haes->instance))->IER, interrupt);
}
